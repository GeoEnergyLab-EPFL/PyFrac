...
# internal imports
from pyfrac.level_set.discontinuous_front_reconstruction import reconstruct_front_LS_gradient
from pyfrac.tip.volume_integral import Integral_over_cell
from pyfrac.mesh_obj.mesh import get_8neighbors
from pyfrac.utilities.postprocess_fracture import get_l_Ffront_ordered_as_v
...
def process_closer_to_done(still_to_be_done, done, mesh, K1c, elts, k):
    ...
    for i in still_to_be_done:
        ...
        if len(nei_done)>0:
            indexes = np.flatnonzero(np.isin(elts, nei_done))
            if k < 1:
...
def get_fracture_size_dependent_toughness(Ffront, EltTip, velocity, mesh, params):
...
def get_fracture_velocity_dependent_toughness(EltTip, velocity, params):
    ...
    for i in range(len(EltTip)):
        Kvel = B * velocity[i] ** params[2]
        K1c[i] = min([Kvel, params[4] * params[3]])

    # * --- We have to average the toughness as it is a global property
    return np.mean(K1c)
