...
class fmm:
    ...
    def calcLs(self, calcLS, mesh):
        ...
        with warnings.catch_warnings():
            ...
            newLS[mask] = np.asarray(
                (theta_1 + beta**2 * theta_2 + theta_sq**0.5) / (1 + beta**2)
            )[mask]
            newLS[nmask] = np.asarray([theta_2 + mesh.hy, theta_1 + mesh.hx]).min(
                axis=0
            )[nmask]
        ...
        while len(self.heapStruct) != 0:
            ...
            if updN.any():
                ...
                if toUpdate.any():
                    ...
                    updH = np.where(
                        np.isin(
                            list(zip(*self.heapStruct))[1], newNLS[1][updN][toUpdate]
                        )
                    )[0]
                    ...
                    toUpdate = list(
                        map(
                            ...
                            np.asarray(
                                (
                                    self.LS[newNLS[1][updN][toUpdate]],
                                    newNLS[1][updN][toUpdate],
                                )
                            ).T,
                        )
                    )
            ...
            if newN.any():
                ...
                toPush = list(
                    map(tuple, np.asarray((newNLS[0][newN], newNLS[1][newN])).T)
                )
                for item in toPush:
