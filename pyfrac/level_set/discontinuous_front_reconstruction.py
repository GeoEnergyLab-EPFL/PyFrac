...
def reconstruct_front_LS_gradient(dist, EltBand, EltChannel, mesh):
    ...
    for i in range(0, len(EltRest)):
        ...
        if Pdis >= 0:
            ...
            #     0         1
            if (
                dist[neighbors_tip[0]] <= dist[neighbors_tip[1]]
                and dist[neighbors_tip[2]] <= dist[neighbors_tip[3]]
            ):
                # if zero vertex is 0:
                gradx = (
                    -(
                        (dist[neighbors_tip[0]] + dist[neighbors_tip[4]]) / 2
                        - (dist[EltRest[i]] + dist[neighbors_tip[2]]) / 2
                    )
                    / mesh.hx
                )
                grady = (
                    (dist[neighbors_tip[0]] + dist[EltRest[i]]) / 2
                    - (dist[neighbors_tip[4]] + dist[neighbors_tip[2]]) / 2
                ) / mesh.hy
                ...
                # if zero vertex is 1:
                gradx = (
                    (dist[neighbors_tip[1]] + dist[neighbors_tip[5]]) / 2
                    - (dist[EltRest[i]] + dist[neighbors_tip[2]]) / 2
                ) / mesh.hx
                grady = (
                    (dist[neighbors_tip[1]] + dist[EltRest[i]]) / 2
                    - (dist[neighbors_tip[5]] + dist[neighbors_tip[2]]) / 2
                ) / mesh.hy
                ...
                # if zero vertex is 2:
                gradx = (
                    (dist[neighbors_tip[1]] + dist[neighbors_tip[7]]) / 2
                    - (dist[EltRest[i]] + dist[neighbors_tip[3]]) / 2
                ) / mesh.hx
                grady = (
                    -(
                        (dist[neighbors_tip[1]] + dist[EltRest[i]]) / 2
                        - (dist[neighbors_tip[3]] + dist[neighbors_tip[7]]) / 2
                    )
                    / mesh.hy
                )
                ...
                gradx = (
                    -(
                        (dist[neighbors_tip[6]] + dist[neighbors_tip[0]]) / 2
                        - (dist[EltRest[i]] + dist[neighbors_tip[3]]) / 2
                    )
                    / mesh.hx
                )
                grady = (
                    (dist[neighbors_tip[0]] + dist[EltRest[i]]) / 2
                    - (dist[neighbors_tip[6]] + dist[neighbors_tip[3]]) / 2
                ) / mesh.hy

            alpha = np.append(
                alpha, np.abs(np.arcsin(grady / (gradx**2 + grady**2) ** 0.5))
            )
...
def UpdateLists(EltsChannel, EltsTipNew, FillFrac, levelSet, mesh):
    ...
    while i < len(
        eltsTip
    ):  # to remove a special case encountered in sharp edges and rectangular cells
        neighbors = mesh.NeiElements[eltsTip[i]]
        if inTip[neighbors[0]] and inTip[neighbors[3]] and inTip[neighbors[3] - 1]:
            conjoined = np.asarray(
                [neighbors[0], neighbors[3], neighbors[3] - 1, eltsTip[i]]
            )
    ...
    zeroVrtx = np.zeros(
        (len(eltsTip),), int
    )  # Vertex from where the perpendicular is drawn
