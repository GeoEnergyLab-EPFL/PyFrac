...
class Hdot_3DR0opening(LinearOperator):
    ...
    def set(self, data):
        if BEMatrix is None:
            raise ImportError(
                "bigwham4py is required to use Hdot_3DR0opening but is not installed. "
                "Install it with 'pip install bigwham4py'."
            )
        ...
        coor3D = (
            coor3D.flatten()
        )  # coor : is an array with the coordinates of all the vertexes of the elements of the mesh
        conn3D = (
            conn.flatten()
        )  # conn : is an array with all the connectivity of the mesh
        ...
        self.hmat_base = BEMatrix(
            ...
            eta,
            eps_aca,
            n_openMP_threads=9999,
        )
        self.HMATtract = self.hmat_base

        self.compressionratio = self.HMATtract.getCompression()

        self._set_domain_and_codomain_IDX(
            np.arange(self.HMAT_size_), np.arange(self.HMAT_size_)
        )
    ...
    def _matvec(self, uk):
        ...
        # traction = self.hmat_base @ uk_full
        # y_via_full = traction[self.codomain_INDX]

        y = self.HMATtract @ uk

        # print(f"{np.allclose(y_via_full, y)=}")
        # y = y_via_full
    ...
    def _set_domain_and_codomain_IDX(self, domainIDX, codomainIDX):
        ...
        # print(f"{self.hmat_base.shape=}")
        # print(f"{codomainIDX.shape=}")
        # print(f"{domainIDX.shape=}")
        # print(f"{codomainIDX=}")
        # print(f"{domainIDX=}")

        self.HMATtract = self.hmat_base[codomainIDX, domainIDX]
        self.shape = self.HMATtract.shape

        # print(f"Bigwham hmat selection, new shape = {self.shape}")
