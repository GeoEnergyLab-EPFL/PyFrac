...
# local imports
from pyfrac.solid.elasticity_isotropic_HMAT_hook import Hdot_3DR0opening
from pyfrac.solid.elasticity_tip_correction import tip_correction_factors
from pyfrac.utilities.utility import append_new_line
...
@njit(
    parallel=True, cache=True, nogil=True, fastmath=True
...
def matvec_fast(uk, elemX, elemY, dimY, nx, C_toeplitz_coe, C_precision):
    ...
    for iter1 in prange(dimY):
        ...
        res[iter1] = np.dot(
            C_toeplitz_coe[np.abs(jX - jY[iter1]) + np.abs(iX - iY[iter1])], uk
        )
    return res
    ...
    if dimX == 0 or dimY == 0:
        ...
        if dimX != dimY:
            ...
            for iter1 in prange(dimY):
                i1 = iY[iter1]
                j1 = jY[iter1]
                C_sub[iter1, 0:dimX] = localC_toeplotz_coe[
                    np.abs(j1 - jX) + nx * np.abs(i1 - iX)
                ]
            return C_sub
...
)  # <-- here parallel can not be set to True because currently appending to list is not threadsafe
def getSuperFast_sparseC_smooth(
    C_toeplitz_coe, C_toeplitz_coe_decay, elmts, nx, decay_tshold=0.9, probability=0.05
):
    ...
    for iter1 in prange(dimX):
        ...
        for iter2 in range(iter1):
            ii2 = index[iter2]
            if (
                C_toeplitz_coe_decay[ii2] > decay_tshold
    ...
    for iter1 in prange(dimX):
        ...
        for iter2 in range(iter1):
            ...
            if ii2_a < 2 and ii2_b < 2 and ii2_a > -2 and ii2_b > -2:
                ...
                data.append(
                    coeff9stencilC[
                        ([[0, 1, 2], [3, 4, 5], [6, 7, 8]])[ii2_a + 1][ii2_b + 1]
                    ]
                )
...
def getFast_sparseC(coeff9stencilC, elmts, nx):
    ...
    for ii in range(dimX):
        cumulative_index = int(
            cumulative_index + 2 * (np.sum(interactions_BOOL[ii, :]) - 1)
        )
...
class elasticity_matrix_toepliz(LinearOperator):
    ...
    def __init__(
        ...
        self.HMAT_tshold = -1  # size of the dot product above which to use the HMAT
        if useHMATdot:
        ...
        # ---- TIP CORRECTION ----
        self.enable_tip_corr = (
            False  # one needs to specifically activate it in case it is needed
        )
        self.EltTip = None  # list of tip elem. IDs
        self.tipcorr = (
            None  # vector of size & ordering as self.EltTip with the correction factors
        )
        self.tips_in_domain_idxs = None  # list of indexes of tip elem. in the domain
        self.tips_in_codomain_idxs = (
            None  # list of indexes of tip elem. in the codomain
        )
        self.tip_in_domain_and_codomain = (
            None  # list of tip elem. IDs in both the codomain and domain
        )
    ...
    def reload(self, Mesh, len_eltcrack=0.0):
        hx = Mesh.hx
        hy = Mesh.hy
        self.hx = hx
        self.hy = hy
        ...
        self.diag_val, self.C_toeplitz_coe = self.reload_toepliz_Coe(
            Lx, Ly, nx, ny, hx, hy, self.mat_prop
        )
    ...
    def reload_HMAT_Coe(self, Mesh, self_eff=None):
        ...
        if self.updateHMATuponRemeshing:
            ...
            if self.kerneltype == "Isotropic_R0":
                ...
                SystemExit(
                    "HMAT not implemented for kerneltype different from Isotropic_R0"
                )
    ...
    def reload_dot(self, Mesh):
        ...
        if len(self.C_toeplitz_coe.shape) == 1:
            ...
            self.C_toeplitz_coe_decay = List(
                C_toeplitz_coe_exp.tolist()
            )  # between 0 and 1
        else:
        ...
        # coeff9stencilC = [diag, dy, diag, dx, self, dx, diag, dy, diag]
        self.coeff9stencilC = np.asarray(
            [
        ...
        self._set_domain_and_codomain_IDX(
            np.arange(self.C_size_),
            np.arange(self.C_size_),
            same_domain_and_codomain=True,
        )
    ...
    def __getitem__(self, elementsXY):
        ...
        C_local = copy.deepcopy(
            self.f_getFast(
                columns, rows, self.nx, self.C_toeplitz_coe, self.C_precision
            )
        )
        ...
        if self.enable_tip_corr:
            ...
            if test_SDandC:
                tips_in_columns_boolean = np.isin(
                    columns, self.EltTip, assume_unique=True
                )
                tips_in_columns_idxs = np.nonzero(tips_in_columns_boolean)
                tips_in_columns_names = columns[tips_in_columns_idxs]
                C_local[tips_in_columns_idxs, tips_in_columns_idxs] = (
                    self.diag_val + self.tipcorr[tips_in_columns_names]
                )
            else:
                common_rows_columns = np.intersect1d(rows, columns)
                common_rows_columns_tips = np.intersect1d(
                    common_rows_columns, self.EltTip, assume_unique=True
                )
                if len(common_rows_columns_tips) > 0:
                    # mind that the tips in rows and columns might be in different order within each array
                    tips_in_columns_idxs = np.where(
                        columns == common_rows_columns_tips[:, None]
                    )[1]
                    tips_in_rows_idxs = np.where(
                        rows == common_rows_columns_tips[:, None]
                    )[1]
                    ...
                    C_local[tips_in_rows_idxs, tips_in_columns_idxs] = (
                        self.diag_val + self.tipcorr[common_rows_columns_tips]
                    )
    ...
    def _matvec(self, uk):
        if not self.enable_tip_corr and not self.left_precJ and not self.right_precJ:
            ...
            if self.left_precJ:
                ...
                # todo: check tip correction as when I wrote this message I found that np.intersect1d is sorting the result and it created a bug for such reason
                # todo: self.tipINDX_codomain is not defined anymore
                res[self.tips_in_codomain_idxs] = (
                    res[self.tips_in_codomain_idxs]
                    + uk[self.tips_in_domain_idxs]
                    * self.tipcorr[self.tip_in_domain_and_codomain]
                )
    ...
    def _precJm1dotvec(self, uk, tipINDX_codomain=None):
        ...
        Pm1_vec = np.full(len(uk), 1.0 / self.diag_val)
        if tipINDX_codomain is None:
            # find the positions of the tip elements in the codomain
            # todo: check tip correction as when I wrote this message I found that np.intersect1d is sorting the result and it created a bug for such reason
            # todo: self.tipINDX_codomain is not defined anymore
            waste, tipINDX_codomain, waste = np.intersect1d(
                self.codomain_INDX, self.EltTip, assume_unique=True, return_indices=True
            )
        Pm1_vec[tipINDX_codomain] = 1.0 / (self.diag_val + self.tipcorr[self.EltTip])
        return Pm1_vec * uk
    ...
    def _precJdotvec(self, uk, tipINDX_codomain=None):
        ...
        if tipINDX_codomain is None:
            ...
            tips_in_codomain_idxs = np.where(
                self.codomain_INDX == self.EltTip[:, None]
            )[1]
        Pm1_vec[tipINDX_codomain] = self.diag_val + self.tipcorr[self.EltTip]
        return Pm1_vec * uk

    def _set_domain_and_codomain_IDX(
        self, domainIDX, codomainIDX, same_domain_and_codomain=False
    ):
        """
        General example:
        ...
        if self.enable_tip_corr:
            self._set_tip_el_in_codomain(
                domainIDX, codomainIDX, same_domain_and_codomain
            )
        else:
            self.tips_in_domain_idxs = []  # list of indexes of tip elem. in the domain
            self.tips_in_codomain_idxs = (
                []
            )  # list of indexes of tip elem. in the codomain
            self.tip_in_domain_and_codomain = (
                []
            )  # list of tip elem. IDs in both the codomain and domain

        if self.useHMATdot:
            self.HMAT._set_domain_and_codomain_IDX(domainIDX, codomainIDX)

    def _set_tip_el_in_codomain(
        self, domain_INDX, codomain_INDX, same_domain_and_codomain
    ):
        """
        General example:
        domain indexes are [1 , 2] of NON ZERO elements used to make the dot product
        ...
        if same_domain_and_codomain:
            tips_in_codomainIDX_boolean = np.isin(
                codomainIDX, self.EltTip, assume_unique=True
            )
            ...
            common_codomain_domain_tips = np.intersect1d(
                common_codomain_domain, self.EltTip, assume_unique=True
            )
            if len(common_codomain_domain_tips) > 0:
                # mind that the tips in rows and columns might be in different order within each array
                tips_in_domain_idxs = np.where(
                    domain_INDX == common_codomain_domain_tips[:, None]
                )[1]
                tips_in_codomain_idxs = np.where(
                    codomain_INDX == common_codomain_domain_tips[:, None]
                )[1]
    ...
    def _get9stencilC(self, elmts, decay_tshold=0.9, probability=0.15):
        ...
        data, rows, cols, dimX = self.f_getFast_sparseC(
            self.coeff9stencilC, elmts, self.nx
        )
        ...
        return coo_matrix(
            (data, (rows, cols)), shape=(dimX, dimX), dtype=self.C_precision
        ).tocsc()
    ...
    def _set_kerneltype_as_it_used_to_be(self):
        from pyfrac.solid.elasticity_kernels.isotropic_R4_elem import (
            matvec_fast_R4,
            getFast_R4,
            getFast_sparseC_R4,
        )
