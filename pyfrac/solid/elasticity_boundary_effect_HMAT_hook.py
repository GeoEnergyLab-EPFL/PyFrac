...
# Internal imports
from pyfrac.utilities.utility import getMemUse
...
def deleteNonUsedRows(row_ind, col_ind, values, toBeSavedROWs): #todo: this method looks too fill up the memory (~HMAT)
    ...
    # save_those_bin = np.isin(row_indexes, save_those, assume_unique=False)
    # final = row_indexes[save_those_bin]
    #####################################
    toBeSavedRows_bin = np.isin(row_ind, toBeSavedROWs, assume_unique=False)
...
class Hdot(LinearOperator):
  ...
  """
  def __init__(self):
      from pyfrac.solid.pypart import Bigwhamio
  ...
  def set(self, data):
    from pyfrac.solid.pypart import pyGetFullBlocks
