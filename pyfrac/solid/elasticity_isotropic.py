...
# external imports
import numpy as np
from pyfrac.mesh_obj.mesh import Neighbors
from numba.typed import List

# internal imports
from pyfrac.solid.elasticity_toeplitz import elasticity_matrix_toepliz, matvec_fast, getFast, getSuperFast_sparseC
from pyfrac.solid.elasticity_kernels.isotropic_R0_elem import get_toeplitzCoe_isotropic, get_R0_normal_traction_at
from pyfrac.solid.elasticity_kernels.isotropic_R4_elem import get_toeplitzCoe_isotropic_R4, get_R4_normal_traction_at
from pyfrac.solid.elasticity_kernels.isotropic_R4_elem import matvec_fast_R4, getFast_R4, getFast_sparseC_R4

# -----------------------------------------------------------------------------------------------------------------------

class load_isotropic_elasticity_matrix_toepliz(elasticity_matrix_toepliz):

    def __init__(self, Mesh, Ep, C_precision=np.float64, useHMATdot=False, nu=0.0, HMATparam = None, Kernel='R0'):
