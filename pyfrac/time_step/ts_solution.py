...
from pyfrac.level_set.continuous_front_reconstruction import (
    you_advance_more_than_2_cells,
)
from pyfrac.utilities.labels import TS_errorMessages
from pyfrac.time_step.ts_explicit_front import time_step_explicit_front
from pyfrac.time_step.ts_implicit_front.inj_extended_footprint import (
    injection_extended_footprint,
)
from pyfrac.time_step.ts_implicit_front.inj_same_footprint import (
    injection_same_footprint,
)
from pyfrac.level_set.level_set_utils import get_front_region
from pyfrac.level_set.anisotropy import (
    projection_from_ribbon_LS_gradient_at_tip,
    get_toughness_from_cellCenter_iter,
)
from pyfrac.properties import IterationProperties, instrument_start, instrument_close
from pyfrac.level_set.anisotropy import (
    get_fracture_size_dependent_toughness,
    get_fracture_velocity_dependent_toughness,
)

# ----------------------------------------------------------------------------------------------------------------------


def attempt_time_step(
    Frac,
    C,
    Boundary,
    mat_properties,
    ...
    if inj_properties.delayed_second_injpoint_elem is not None:
        if inj_properties.rate_delayed_inj_pt_func is None:
            if Frac.time >= inj_properties.injectionTime_delayed_second_injpoint:
                Qin[inj_properties.delayed_second_injpoint_elem] = (
                    inj_properties.injectionRate_delayed_second_injpoint
                    / len(inj_properties.delayed_second_injpoint_elem)
                )
            else:
                Qin[inj_properties.delayed_second_injpoint_elem] = (
                    inj_properties.init_rate_delayed_second_injpoint
                    / len(inj_properties.delayed_second_injpoint_elem)
                )
        else:
            Qin[inj_properties.delayed_second_injpoint_elem] = (
                inj_properties.rate_delayed_inj_pt_func(Frac.time)
                / len(inj_properties.delayed_second_injpoint_elem)
            )
    ...
    if (
        mat_properties.sizeDependentToughness[0]
        or mat_properties.velocityDependentToughness[0]
    ):
        if mat_properties.sizeDependentToughness[0]:
            K_Ic_current = (32 / np.pi) ** 0.5 * get_fracture_size_dependent_toughness(
                Frac.Ffront,
                Frac.EltTip,
                Frac.v,
                Frac.mesh,
            ...
            log.debug(
                "The current KIc is: " + str(K_Ic_current / ((32 / np.pi) ** 0.5))
            )
        elif mat_properties.velocityDependentToughness[0]:
            K_Ic_current = (
                32 / np.pi
            ) ** 0.5 * get_fracture_velocity_dependent_toughness(
                Frac.EltTip, Frac.v, mat_properties.velocityDependentToughness
            )

        if K_Ic_current < 0.95 * mat_properties.Kprime[0]:
            K_Ic_current = 0.95 * mat_properties.Kprime[0]
            log.debug(
                "Limiting the toughness reduction to 5%: "
                + str(K_Ic_current / ((32 / np.pi) ** 0.5))
            )

        mat_properties.K1c = (
            K_Ic_current
            / ((32 / np.pi) ** 0.5)
            * np.ones((Frac.mesh.NumberOfElts,), float)
        )
    ...
    if sim_properties.frontAdvancing == "explicit":
        ...
        exitstatus, Fr_k = time_step_explicit_front(
            Frac,
            C,
            Boundary,
            timeStep,
        ...
        if exitstatus == 1:
            if (
                ...
                and sim_properties.limitAdancementTo2cells
            ):
                exitstatus = 17
        ...
        exitstatus, Fr_k = injection_same_footprint(
            Frac,
            C,
            Boundary,
            timeStep,
    ...
    if mat_properties.inv_with_heter_K1c:
        front_region = get_front_region(
            Fr_k.mesh, Fr_k.EltRibbon, Fr_k.sgndDist[Fr_k.EltRibbon]
        )
        alpha_ribbon = projection_from_ribbon_LS_gradient_at_tip(
            Fr_k.EltRibbon,
            front_region,
            Fr_k.mesh,
            Fr_k.sgndDist,
        ...
        Kprime_k = get_toughness_from_cellCenter_iter(
            alpha_ribbon, Fr_k.mesh.CenterCoor[Fr_k.EltRibbon], mat_properties
        )
        stagnant_crt[
            np.where(
                Kprime_k.of(
                    Fr_k.sgndDist[Fr_k.EltRibbon], mesh=Fr_k.mesh, ribbon=Fr_k.EltRibbon
                )
        ...
        stagnant_crt[
            np.where(
                mat_properties.Kprime[Fr_k.EltRibbon]
                * (-Fr_k.sgndDist[Fr_k.EltRibbon]) ** 0.5
                / (mat_properties.Eprime * Fr_k.w[Fr_k.EltRibbon])
                > 1
            )[0]
    ...
    if np.all(stagnant):
        ...
        sumQin_gt_0 = np.sum(Qin) * timeStep != 0.0
        dw_perc_w = np.max(delta_w) / np.max(Fr_k.w) < 5.0 / 100.0
        hard_dw_threshold = np.max(delta_w) < sim_properties.tolerancewIncr
        if sumQin_gt_0 and (dw_perc_w or hard_dw_threshold):
            log.critical(
                f"The time step is too small to induce a significant change in opening (max dw = {np.max(delta_w)})"
            )
            if sim_properties.get_volumeControl():
                ...
                log.critical(
                    "   nevertheless the time step will be accepted...continuing"
                )
                return 1, Fr_k
        else:
            return 1, Fr_k
    ...
    while (
        norm > sim_properties.tolFractFront or force_1loop
        ...
        exitstatus, Fr_k = injection_extended_footprint(
            Fr_k.w,
            Frac,
            C,
            Boundary,
        ...
        # from pyfrac.level_set.continuous_front_reconstruction import plot_two_fronts
        ...
        if exitstatus == 1:
            ...
            if not k == 1:
                ...
                norm_first_it = abs(
                    (sum(Fr_k.FillF) - sum(fill_frac_last)) / len(Fr_k.FillF)
                )
        else:
            norm = np.nan
        ...
        if not k == 2:
            ...
            if norm_first_it is None:
                log.debug(
                    "Norm of subsequent filling fraction estimates = " + repr(norm)
                )
            else:
                log.debug(
                    "Norm of subsequent filling fraction estimates = "
                    + repr(norm_first_it)
                    + " forcing 2 iter min."
                )
        ...
        if norm is not np.nan:
            if abs((previous_norm - norm) / norm) < 0.0001 and k > 35:
                log.debug(
                    "Norm of subsequent Norms of subsequent filling fraction estimates = "
                    + str(abs((previous_norm - norm) / norm))
                    + " < 0.0001"
                )
                exitstatus = 15
        ...
        if (
            k > 10 and inj_properties.modelInjLine
        ):  # this is a value coming from experience
            model = np.polyfit(
                np.arange(k - 1), dwMAX_history[1:k], 1
            )  # linear regression
            if (len(np.unique(norm_history)) < 6 and model[0] > -0.0001) or model[
                0
            ] > 0.0:  # these are a values coming from experience
                log.critical(
                    "The iteration on the front position is stacked in a loop --> forcing the fracture to be stagnant. "
        ...
        if sim_properties.get_volumeControl():
            ...
            if norm < sim_properties.tolFractFront and loop_already_forced:
                ...
                log.debug(
                    " --> Forcing one more loop to see if the convergence has been really achieved"
                )
                loop_already_forced = True
                force_1loop = True
            elif norm >= sim_properties.tolFractFront and loop_already_forced:
                log.debug(
                    " --> convergence not achieved, iterate more if other conditions allows for that"
                )
                loop_already_forced = False
                force_1loop = False
            elif norm >= sim_properties.tolFractFront and not loop_already_forced:
    ...
    log.debug(
        "Fracture front converged after "
        + repr(k)
        + " iterations with norm = "
        + repr(norm)
    )
