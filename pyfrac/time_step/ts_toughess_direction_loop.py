...
# Internal Imports
from pyfrac.level_set.anisotropy import projection_from_ribbon_LS_gradient_at_tip
from pyfrac.level_set.anisotropy import projection_from_ribbon
from pyfrac.level_set.anisotropy import get_toughness_from_cellCenter_iter
...
from pyfrac.tip.tip_inversion import TipAsymInversion
from pyfrac.level_set.FMM import fmm


def toughness_direction_loop(
    w_k,
    sgndDist_k,
    Fr_lstTmStp,
    sim_properties,
    ...
    while itr < sim_properties.maxProjItrs:
        ...
        if (
            ...
            or mat_properties.TI_elasticity
            ...
            if itr == 0:
                # first iteration
                alpha_ribbon_k = projection_method(
                ...
                alpha_ribbon_km1 = np.zeros(
                    Fr_lstTmStp.EltRibbon.size,
                )
            else:
                alpha_ribbon_k = (
                    1 - relax_factor
                ) * alpha_ribbon_k + relax_factor * projection_method(
                    Fr_lstTmStp.EltRibbon,
                    second_arg,
                    Fr_lstTmStp.mesh,
                    sgndDist_k,
        ...
        if mat_properties.TI_elasticity:  # and not mat_properties.inv_with_heter_K1c:
            Eprime_k = TI_plain_strain_modulus(alpha_ribbon_k, mat_properties.Cij)
        ...
        sgndDist_k = 1e50 * np.ones(
            (Fr_lstTmStp.mesh.NumberOfElts,), float
        )  # Initializing the cells with extremely

        perfNode_tipInv = instrument_start("tip inversion", perfNode)

        sgndDist_k[Fr_lstTmStp.EltRibbon] = -TipAsymInversion(
            w_k,
            Fr_lstTmStp,
            mat_properties,
            fluid_properties,
        ...
        sgndDist_k[Fr_lstTmStp.EltRibbon] = np.minimum(
        ...
        front_region = get_front_region(
            Fr_lstTmStp.mesh, Fr_lstTmStp.EltRibbon, sgndDist_k[Fr_lstTmStp.EltRibbon]
        )
        ...
        eval_region = np.where(sgndDist_k[front_region] >= -Fr_lstTmStp.mesh.cellDiag)[
            0
        ]
        ...
        if (
            ...
            or sim_properties.explicitProjection
        ):
            break

        norm = np.linalg.norm(
            np.abs(np.sin(alpha_ribbon_k) - np.sin(alpha_ribbon_km1))
            + np.abs(np.cos(alpha_ribbon_k) - np.cos(alpha_ribbon_km1))
        )
        if norm < sim_properties.toleranceProjection:
            log.debug(
                "projection iteration converged after "
                + repr(itr - 1)
                + " iterations; exiting norm "
                + repr(norm)
            )
            break
