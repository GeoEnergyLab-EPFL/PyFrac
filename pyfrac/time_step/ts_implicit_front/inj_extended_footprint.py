...
from pyfrac.properties import instrument_start
...
def injection_extended_footprint(
    w_k,
    Fr_lstTmStp,
    C,
    Boundary,
        ...
        while not correct_size_of_pstv_region[0]:
            ...
            if not correct_size_of_pstv_region[0]:
                ...
                fmmStruct.solveFMM(
                    ...
                    np.unique(np.hstack((pstv_region, Fr_lstTmStp.EltRibbon))),
                    Fr_lstTmStp.mesh,
                )
    ...
    smallNgtvWTip = np.where(np.logical_and(wTip < 0.0, wTip > -1.0e-4 * np.mean(wTip)))
    ...
    if (wTip < 0.0).any() or sum(wTip) == 0.0:
        exitstatus = 4
        return exitstatus, None
    ...
    if sum(mat_properties.Cprime[EltsTipNew]) > 0.0:
    ...
    if sum(mat_properties.Cprime[Fr_lstTmStp.EltChannel]) > 0.0:
    ...
    if np.any(Vel_k[new_channel] <= 0.0):  # negative velocity set to zero...
    ...
    if Boundary is not None:
        ...
        Fr_kplus1.pNet[EltCrack_k] = (
            ...
            - Boundary.last_traction[EltCrack_k]
        )
    else:
        Fr_kplus1.pNet[EltCrack_k] = (
