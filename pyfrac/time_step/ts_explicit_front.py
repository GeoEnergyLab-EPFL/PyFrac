...
from pyfrac.level_set.level_set_utils import get_front_region
from pyfrac.level_set.discontinuous_front_reconstruction import (
    reconstruct_front,
    UpdateLists,
)
from pyfrac.level_set.continuous_front_reconstruction import (
    reconstruct_front_continuous,
    UpdateListsFromContinuousFrontRec,
)
from pyfrac.level_set.anisotropy import (
    reconstruct_front_LS_gradient,
    get_toughness_from_Front,
)
from pyfrac.level_set.anisotropy import find_zero_vertex
from pyfrac.level_set.anisotropy import get_toughness_from_zeroVertex
from pyfrac.time_step.ts_toughess_direction_loop import toughness_direction_loop

from pyfrac.tip.volume_integral import Integral_over_cell
from pyfrac.tip.volume_integral import (
    leak_off_stagnant_tip,
    find_corresponding_ribbon_cell,
)
from pyfrac.tip.tip_inversion import StressIntensityFactor

from pyfrac.systems.sol_sys_dispatcher import solve_width_pressure
from pyfrac.systems.make_sys_common_fun import (
    calculate_fluid_flow_characteristics_laminar,
)
from pyfrac.properties import IterationProperties, instrument_start
from pyfrac.fluid.reyNumb import turbulence_check_tip
from pyfrac.utilities.postprocess_fracture import append_to_json_file
from pyfrac.solid.elasticity_Transv_Isotropic import TI_plain_strain_modulus

# ----------------------------------------------------------------------------------------------------------------------


def time_step_explicit_front(
    Fr_lstTmStp,
    C,
    Boundary,
    timeStep,
    ...
    fmmStruct.solveFMM(
        (
            Fr_lstTmStp.sgndDist[Fr_lstTmStp.EltTip] - (timeStep * Fr_lstTmStp.v),
            Fr_lstTmStp.EltTip,
        ),
        np.unique(np.hstack((pstv_region, Fr_lstTmStp.EltTip))),
        Fr_lstTmStp.mesh,
    )
    ...
    if sim_properties.projMethod == "ILSA_orig":
        EltsTipNew, l_k, alpha_k, CellStatus = reconstruct_front(
            sgndDist_k, front_region, Fr_lstTmStp.EltChannel, Fr_lstTmStp.mesh
        )
    elif sim_properties.projMethod == "LS_grad":
        EltsTipNew, l_k, alpha_k, CellStatus = reconstruct_front_LS_gradient(
            sgndDist_k, front_region, Fr_lstTmStp.EltChannel, Fr_lstTmStp.mesh
        )
        ...
        while not correct_size_of_pstv_region[0]:
            ...
            if not correct_size_of_pstv_region[0]:
                ...
                fmmStruct.solveFMM(
                    (
                        Fr_lstTmStp.sgndDist[Fr_lstTmStp.EltTip]
                        - (timeStep * Fr_lstTmStp.v),
                        Fr_lstTmStp.EltTip,
                    ),
                    np.unique(np.hstack((pstv_region, Fr_lstTmStp.EltTip))),
                    Fr_lstTmStp.mesh,
                ...
                eval_region = np.where(
                    sgndDist_k[front_region] >= -Fr_lstTmStp.mesh.cellDiag
                )[0]
    ...
    if not np.isin(EltsTipNew, front_region).any():
        raise SystemExit(
            "The tip elements are not in the band. Increase the size of the band for FMM to evaluate"
            " level set."
        )
    ...
    if (
        len(InCrack_k[np.where(InCrack_k == 1)]) > sim_properties.maxElementIn
        and sim_properties.meshReductionPossible
    ):
        exitstatus = 16
        return exitstatus, Fr_lstTmStp
    ...
    FillFrac_k = (
        Integral_over_cell(
            ...
            Fr_lstTmStp.mesh,
            "A",
            projMethod=sim_properties.projMethod,
        )
    ...
    if sim_properties.projMethod != "LS_continousfront":
        ...
        (
            ...
            Fr_lstTmStp.EltChannel, EltsTipNew, FillFrac_k, sgndDist_k, Fr_lstTmStp.mesh
        )
    ...
    partlyFilledTip = np.arange(EltsTipNew.shape[0])[np.isin(EltsTipNew, EltTip_k)]
    ...
    corr_ribbon = find_corresponding_ribbon_cell(
        EltsTipNew, alpha_k, zrVrtx_newTip, Fr_lstTmStp.mesh
    )
    Cprime_tip = mat_properties.Cprime[corr_ribbon]

    if (
        sim_properties.paramFromTip
        or mat_properties.anisotropic_K1c
        or mat_properties.inv_with_heter_K1c
    ):
        if (
            not mat_properties.sizeDependentToughness[0]
            and not mat_properties.velocityDependentToughness[0]
        ):
            if sim_properties.projMethod != "LS_continousfront":
                # get toughness from zero vertex
                Kprime_tip = (32 / np.pi) ** 0.5 * get_toughness_from_zeroVertex(
                    EltsTipNew,
                    Fr_lstTmStp.mesh,
                    mat_properties,
                    alpha_k,
                ...
                Kprime_tip = (32.0 / np.pi) ** 0.5 * get_toughness_from_Front(
                    Ffront,
                    EltsTipNew,
                    EltTip_k,
                    fully_traversed_k,
    ...
    if mat_properties.TI_elasticity:
        Eprime_tip = TI_plain_strain_modulus(alpha_k, mat_properties.Cij)
    else:
        Eprime_tip = np.full(
            (EltsTipNew.size,), mat_properties.Eprime, dtype=np.float64
        )
    ...
    stagnant = (
        -(sgndDist_k[EltsTipNew] - Fr_lstTmStp.sgndDist[EltsTipNew])
        / Fr_lstTmStp.mesh.cellDiag
        < sim_properties.toleranceVStagnant
    )
    ...
    if sum(mat_properties.Cprime[EltsTipNew]) > 0:
        ...
        LkOff[EltsTipNew] = (
            2
            * mat_properties.Cprime[EltsTipNew]
            * Integral_over_cell(
                EltsTipNew,
                alpha_k,
                l_k,
    ...
    if sum(mat_properties.Cprime[Fr_lstTmStp.EltChannel]) > 0:
        t_since_arrival = (
            Fr_lstTmStp.time - Fr_lstTmStp.Tarrival[Fr_lstTmStp.EltChannel]
        )
        t_since_arrival[t_since_arrival < 0.0] = 0.0
        if np.isnan(t_since_arrival).any():
            t_since_arrival[np.argwhere(np.isnan(t_since_arrival))] = 0.0
        if (np.abs(t_since_arrival) == np.inf).any():
            t_since_arrival[np.argwhere(np.abs(t_since_arrival) == np.inf)] = 0.0
        LkOff[Fr_lstTmStp.EltChannel] = (
            2
            * mat_properties.Cprime[Fr_lstTmStp.EltChannel]
            * ((t_since_arrival + timeStep) ** 0.5 - t_since_arrival**0.5)
            * Fr_lstTmStp.mesh.EltArea
        )
        ...
        if stagnant.any():
            LkOff[EltsTipNew[stagnant]] = leak_off_stagnant_tip(
                EltsTipNew[stagnant],
                l_k[stagnant],
                alpha_k[stagnant],
    ...
    if (
        data[0] != None
    ):  # todo: Check why we need this if condition in the case of volume control
        fluidVel = data[0][0]
    ...
    if np.any(Vel_k[new_channel] <= 0.0):  # negative velocitz set to zero...
        Vel_k[new_channel[np.where(Vel_k[new_channel] <= 0.0)]] = 1e-6
        log.debug(
            "Some zero velocities are set to a minimum velocity to evaluate the leak-off."
        )
    t_enter = Fr_lstTmStp.time + timeStep - l_k[new_channel] / Vel_k[new_channel]
    max_l = Fr_lstTmStp.mesh.hx * np.cos(
        alpha_k[new_channel]
    ) + Fr_lstTmStp.mesh.hy * np.sin(alpha_k[new_channel])
    t_leave = (
        Fr_lstTmStp.time + timeStep - (l_k[new_channel] - max_l) / Vel_k[new_channel]
    )
    ...
    if (np.abs(Tarrival_k[EltChannel_k]) == np.inf).any():
        Tarrival_k[
            EltChannel_k[np.argwhere(np.abs(Tarrival_k[EltChannel_k]) == np.inf)]
        ] = max_Tarrival
    ...
    if Boundary is not None:
        ...
        Fr_kplus1.pNet[EltCrack_k] = (
            pf_n_plus1[EltCrack_k]
            - mat_properties.SigmaO[EltCrack_k]
            - Fr_kplus1.boundEffTraction[EltCrack_k]
        )
    else:
        Fr_kplus1.pNet[EltCrack_k] = (
            pf_n_plus1[EltCrack_k] - mat_properties.SigmaO[EltCrack_k]
        )
    ...
    if sim_properties.projMethod != "LS_continousfront":
        ...
        if (
            sim_properties.saveToDisk
            and sim_properties.saveStatisticsPostCoalescence
            and Fr_lstTmStp.number_of_fronts != Fr_kplus1.number_of_fronts
        ):
            myJsonName = sim_properties.set_outputFolder + "_mesh_study.json"
            append_to_json_file(
                myJsonName, Fr_kplus1.mesh.nx, "append2keyAND2list", key="nx"
            )
            append_to_json_file(
                myJsonName, Fr_kplus1.mesh.ny, "append2keyAND2list", key="ny"
            )
            append_to_json_file(
                myJsonName, Fr_kplus1.mesh.hx, "append2keyAND2list", key="hx"
            )
            append_to_json_file(
                myJsonName, Fr_kplus1.mesh.hy, "append2keyAND2list", key="hy"
            )
            append_to_json_file(
                myJsonName,
                Fr_kplus1.EltCrack.size,
                "append2keyAND2list",
                key="elements_in_crack",
            )
            append_to_json_file(
                myJsonName,
                Fr_kplus1.EltTip.size,
                "append2keyAND2list",
                key="elements_in_tip",
            )
            append_to_json_file(
                myJsonName, Fr_kplus1.time, "append2keyAND2list", key="coalescence_time"
            )
    ...
    front_region, eval_region, sgndDist_k = toughness_direction_loop(
        Fr_kplus1.w,
        sgndDist_k,
        Fr_lstTmStp,
        sim_properties,
    ...
    Fr_kplus1.v = (
        -(sgndDist_k[Fr_kplus1.EltTip] - Fr_lstTmStp.sgndDist[Fr_kplus1.EltTip])
        / timeStep
    )
    sgndDist_k[Fr_kplus1.EltTip[Fr_kplus1.v < 0]] = Fr_lstTmStp.sgndDist[
        Fr_kplus1.EltTip[Fr_kplus1.v < 0]
    ]
    ...
    Fr_kplus1.efficiency = (
        Fr_kplus1.mesh.EltArea
        * (
            np.sum(Fr_kplus1.w[Fr_kplus1.EltTip] * Fr_kplus1.FillF)
            + np.sum(Fr_kplus1.w[Fr_kplus1.EltChannel])
        )
    ...
    if fluid_properties.turbulence:
        ...
        if (
            sim_properties.saveFluidFlux
            or sim_properties.saveFluidVel
            or sim_properties.saveReynNumb
            or sim_properties.saveFluidFluxAsVector
            or sim_properties.saveFluidVelAsVector
        ):
