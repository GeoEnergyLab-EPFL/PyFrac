...
# Internal imports
from pyfrac.systems.make_sys_common_fun import get_finite_difference_matrix
from pyfrac.systems.make_sys_common_fun import Gravity_term
...
def MakeEquationSystem_ViscousFluid_pressure_substituted_sparse(solk, interItr, *args):
    ...
    interItr_kp1 = [None] * 4
    FinDiffOprtr = get_finite_difference_matrix(
        wNplusOne,
        solk,
        frac,
        EltCrack,
    ...
    G = Gravity_term(wNplusOne, EltCrack, fluid_prop, frac.mesh, InCrack, sim_prop)
    ...
    ch_AplusCf = dt * FinDiffOprtr.tocsr()[ch_indxs, :].tocsc()[
        :, ch_indxs
    ] - sparse.diags(
        [np.full((n_ch,), fluid_prop.compressibility * wcNplusHalf[to_solve])],
        [0],
        format="csr",
    )
    ...
    A[np.ix_(ch_indxs, tip_indxs)] = (
        -dt * (FinDiffOprtr.tocsr()[ch_indxs, :].tocsc()[:, tip_indxs]).toarray()
    )
    A[np.ix_(ch_indxs, act_indxs)] = (
        -dt * (FinDiffOprtr.tocsr()[ch_indxs, :].tocsc()[:, act_indxs]).toarray()
    )

    A[np.ix_(tip_indxs, ch_indxs)] = -(
        dt * FinDiffOprtr.tocsr()[tip_indxs, :].tocsc()[:, ch_indxs]
    ).dot(C[np.ix_(to_solve, to_solve)])
    A[np.ix_(tip_indxs, tip_indxs)] = (
        -dt * FinDiffOprtr.tocsr()[tip_indxs, :].tocsc()[:, tip_indxs]
        + sparse.diags(
    ...
    A[np.ix_(tip_indxs, act_indxs)] = (
        -dt * (FinDiffOprtr.tocsr()[tip_indxs, :].tocsc()[:, act_indxs]).toarray()
    )

    A[np.ix_(act_indxs, ch_indxs)] = -(
        dt * FinDiffOprtr.tocsr()[act_indxs, :].tocsc()[:, ch_indxs]
    ).dot(C[np.ix_(to_solve, to_solve)])
    A[np.ix_(act_indxs, tip_indxs)] = (
        -dt * (FinDiffOprtr.tocsr()[act_indxs, :].tocsc()[:, tip_indxs]).toarray()
    )
    A[np.ix_(act_indxs, act_indxs)] = (
        -dt * FinDiffOprtr.tocsr()[act_indxs, :].tocsc()[:, act_indxs]
        + sparse.diags(
    ...
    pf_ch_prime = (
        np.dot(C[np.ix_(to_solve, to_solve)], frac.w[to_solve])
    ...
    S[tip_indxs] = (
        -(imposed_val - frac.w[to_impose])
        + dt
        * (FinDiffOprtr.tocsr()[tip_indxs, :].tocsc()[:, ch_indxs]).dot(pf_ch_prime)
        + fluid_prop.compressibility * wcNplusHalf[to_impose] * frac.pFluid[to_impose]
        + dt * G[to_impose]
        + dt * Q[to_impose] / frac.mesh.EltArea
        - LeakOff[to_impose] / frac.mesh.EltArea
    )
    S[act_indxs] = (
        -(wc_to_impose - frac.w[active])
        + dt
        * (FinDiffOprtr.tocsr()[act_indxs, :].tocsc()[:, ch_indxs]).dot(pf_ch_prime)
        + fluid_prop.compressibility * wcNplusHalf[active] * frac.pFluid[active]
        + dt * G[active]
        + dt * Q[active] / frac.mesh.EltArea
        - 0.0 * LeakOff[active] / frac.mesh.EltArea
    )
...
# -----------------------------------------------------------------------------------------------------------------------
# @profile
def MakeEquationSystem_ViscousFluid_pressure_substituted_deltaP_sparse(
    solk, interItr, *args, return_w=False, dtype=np.float64
):
    """
    ...
    # 1
    C_solve_solve = C[
        np.ix_(to_solve, to_solve)
    ]  # storing the references to make it faster in case we use toeplitz
    ...
    S = np.zeros((n_total,), dtype=np.float64)
    pf_ch_prime = (
        np.dot(C_solve_solve, frac.w[to_solve])
        + np.dot(C[np.ix_(to_solve, to_impose)], imposed_val)
        + np.dot(C[np.ix_(to_solve, active)], wNplusOne[active])
        + mat_prop.SigmaO[to_solve]
    )
    ...
    S[ch_indxs] = (
        ch_AplusCf.dot(pf_ch_prime)
        + dt
        * (FinDiffOprtr.tocsr()[ch_indxs, :].tocsc()[:, tip_indxs]).dot(
            frac.pFluid[to_impose]
        )
        + dt
        * (FinDiffOprtr.tocsr()[ch_indxs, :].tocsc()[:, act_indxs]).dot(
            frac.pFluid[active]
        )
        + dt * G[to_solve]
        + dt * Q[to_solve] / frac.mesh.EltArea
        - LeakOff[to_solve] / frac.mesh.EltArea
        + fluid_prop.compressibility * wcNplusHalf[to_solve] * frac.pFluid[to_solve]
        + +(ch_AplusCf.tocsr()[ch_indxs, :].tocsc()[:, ch_indxs]).dot(
            delta_tb[to_solve]
        )
    )
    ...
    S[tip_indxs] = (
        ...
        * (FinDiffOprtr.tocsr()[tip_indxs, :].tocsc()[:, ch_indxs]).dot(pf_ch_prime)
        + dt
        * (FinDiffOprtr.tocsr()[tip_indxs, :].tocsc()[:, tip_indxs]).dot(
            frac.pFluid[to_impose]
        )
        + dt
        * (FinDiffOprtr.tocsr()[tip_indxs, :].tocsc()[:, act_indxs]).dot(
            frac.pFluid[active]
        )
        + dt * G[to_impose]
        + dt * Q[to_impose] / frac.mesh.EltArea
        - LeakOff[to_impose] / frac.mesh.EltArea
        + -dt
        * (FinDiffOprtr.tocsr()[tip_indxs, :].tocsc()[:, ch_indxs]).dot(
            delta_tb[to_solve]
        )
    )
    ...
    S[act_indxs] = (
        ...
        * (FinDiffOprtr.tocsr()[act_indxs, :].tocsc()[:, ch_indxs]).dot(pf_ch_prime)
        + dt
        * (FinDiffOprtr.tocsr()[act_indxs, :].tocsc()[:, tip_indxs]).dot(
            frac.pFluid[to_impose]
        )
        + dt
        * (FinDiffOprtr.tocsr()[act_indxs, :].tocsc()[:, act_indxs]).dot(
            frac.pFluid[active]
        )
        + dt * G[active]
        + dt * Q[active] / frac.mesh.EltArea
        - 0.0 * LeakOff[active] / frac.mesh.EltArea
        + -dt
        * (FinDiffOprtr.tocsr()[act_indxs, :].tocsc()[:, ch_indxs]).dot(
            delta_tb[to_solve]
        )
    )
...
def make_local_elast_sys(
    ...
    *args,
    ...
    # A = np.zeros((n_total, n_total), dtype=dtype)
    # a = -time.time()
    ch_AplusCf = dt * FinDiffOprtr[np.ix_(ch_indxs, ch_indxs)] - sparse.diags(
        [np.full((n_ch,), fluid_prop.compressibility * wcNplusHalf[to_solve])],
        [0],
        format="csr",
    )
    ...
    if C.useHMATdot:
        C_loc = C.HMAT.HMATtract._getFullBlocks(keep_ratio=0.25)
    else:
        C_loc = C._get9stencilC(
            to_solve, decay_tshold=decay_tshold, probability=probability
        )
    ...
    *ch_ch*  ch_act    ch_tip
    ...
    # A[ch_indxs, ch_indxs] += np.ones(len(ch_indxs), dtype=dtype)
    A_ch_ch = -((ch_AplusCf).dot(C_loc)) + sparse.identity(
        n_ch, dtype=dtype, format="csc"
    )
    ...
    if n_act == 0 and n_tip == 0:
        ...
        A_temp = (-dt * (FinDiffOprtr[np.ix_(ch_indxs, tip_indxs)])).tocoo(copy=False)
        ...
        *tip_ch*   tip_act   tip_tip
        ...
        A_temp = (
            -(((dt * FinDiffOprtr[np.ix_(tip_indxs, ch_indxs)])).dot(C_loc))
        ).tocoo(copy=False)
        ...
        A_temp = (
            -dt * FinDiffOprtr[np.ix_(tip_indxs, tip_indxs)]
            + sparse.diags(
                [
                    np.full(
                        (n_tip,), fluid_prop.compressibility * wcNplusHalf[to_impose]
                    )
                ],
                [0],
                format="csc",
        ...
        A_temp = (-dt * (FinDiffOprtr[np.ix_(tip_indxs, act_indxs)])).tocoo(copy=False)
        row.extend(A_temp.row + n_ch + n_act)
        ...
        A_temp = (-dt * (FinDiffOprtr[np.ix_(ch_indxs, act_indxs)])).tocoo(copy=False)
        ...
        *act_ch*   act_act     act_tip
        ...
        #                                 ).tocsc()).dot(C_loc)).toarray()
        A_temp = (
            -(((dt * FinDiffOprtr[np.ix_(act_indxs, ch_indxs)])).dot(C_loc))
        ).tocoo(copy=False)
        ...
        A_temp = (-dt * (FinDiffOprtr[np.ix_(act_indxs, tip_indxs)])).tocoo(copy=False)
        row.extend(A_temp.row + n_ch)
        col.extend(A_temp.col + n_ch + n_act)
        data.extend(A_temp.data)
        ...
        #                                                 [0], format='csc')).toarray()
        A_temp = (
            -dt * FinDiffOprtr[np.ix_(act_indxs, act_indxs)]
            + sparse.diags(
                [np.full((n_act,), fluid_prop.compressibility * wcNplusHalf[active])],
                [0],
                format="csc",
    ...
    if n_ch > 0:
        ...
        S[ch_indxs] = (
            ch_AplusCf.dot(pf_ch_prime)
            + dt * (FinDiffOprtr[ch_indxs, :][:, tip_indxs]).dot(frac.pFluid[to_impose])
            + dt * (FinDiffOprtr[ch_indxs, :][:, act_indxs]).dot(frac.pFluid[active])
            + dt * G[to_solve]
            + dt * Q[to_solve] / frac.mesh.EltArea
            - LeakOff[to_solve] / frac.mesh.EltArea
            + fluid_prop.compressibility * wcNplusHalf[to_solve] * frac.pFluid[to_solve]
            + +(ch_AplusCf.tocsr()[ch_indxs, :][:, ch_indxs]).dot(delta_tb[to_solve])
        )
    else:
        pf_ch_prime = []
    ...
    S[tip_indxs] = (
        -(imposed_val - frac.w[to_impose])
        + dt * (FinDiffOprtr[tip_indxs, :][:, ch_indxs]).dot(pf_ch_prime)
        + dt * (FinDiffOprtr[tip_indxs, :][:, tip_indxs]).dot(frac.pFluid[to_impose])
        + dt * (FinDiffOprtr[tip_indxs, :][:, act_indxs]).dot(frac.pFluid[active])
        + dt * G[to_impose]
        + dt * Q[to_impose] / frac.mesh.EltArea
        - LeakOff[to_impose] / frac.mesh.EltArea
        + -dt * (FinDiffOprtr[tip_indxs, :][:, ch_indxs]).dot(delta_tb[to_solve])
    )

    S[act_indxs] = (
        -(wc_to_impose - frac.w[active])
        + dt * (FinDiffOprtr[act_indxs, :][:, ch_indxs]).dot(pf_ch_prime)
        + dt * (FinDiffOprtr[act_indxs, :][:, tip_indxs]).dot(frac.pFluid[to_impose])
        + dt * (FinDiffOprtr[act_indxs, :][:, act_indxs]).dot(frac.pFluid[active])
        + dt * G[active]
        + dt * Q[active] / frac.mesh.EltArea
        - 0.0 * LeakOff[active] / frac.mesh.EltArea
        + -dt * (FinDiffOprtr[act_indxs, :][:, ch_indxs]).dot(delta_tb[to_solve])
    )
...
class EHL_sys_obj(LinearOperator):
    ...
    def _getsys(self, solk, interItr, *args):
        ...
        A, b, interItr, indices, wcNplusHalf, FinDiffOprtr = (
            MakeEquationSystem_ViscousFluid_pressure_substituted_deltaP_sparse(
                solk, interItr, *args, return_w=True
            )
        )
    ...
    def _getsys_simplif(
        self, solk, interItr, *args, decay_tshold=0.9, probability=0.15
    ):
        """
        ...
        A, b, interItr, indices, wcNplusHalf, FinDiffOprtr = make_local_elast_sys(
            solk,
            interItr,
            *args,
            return_w=True,
            decay_tshold=decay_tshold,
    ...
    def _update_sys(self, solk, interItr):
        ...
        wcNplusHalf = (frac.w + wNplusOne) / 2.0
        ...
        if len(active) > 0:
            C._set_domain_and_codomain_IDX(active, to_solve)
            pf_ch_prime = (
                pf_ch_prime + C._matvec(wNplusOne[active]) + mat_prop.SigmaO[to_solve]
            )
        else:
            pf_ch_prime = pf_ch_prime + mat_prop.SigmaO[to_solve]

        S[ch_indxs] = (
            dt * (FinDiffOprtr[ch_indxs, :].tocsc()[:, ch_indxs]).dot(pf_ch_prime)
            - fluid_prop.compressibility * wcNplusHalf[to_solve] * pf_ch_prime
            + dt
            * (FinDiffOprtr[ch_indxs, :].tocsc()[:, tip_indxs]).dot(
                frac.pFluid[to_impose]
            )
            + dt
            * (FinDiffOprtr[ch_indxs, :].tocsc()[:, act_indxs]).dot(frac.pFluid[active])
            + dt * G[to_solve]
            + dt * Q[to_solve] / frac.mesh.EltArea
            - LeakOff[to_solve] / frac.mesh.EltArea
            + fluid_prop.compressibility * wcNplusHalf[to_solve] * frac.pFluid[to_solve]
            + +dt
            * (FinDiffOprtr[ch_indxs, :].tocsc()[:, ch_indxs]).dot(delta_tb[to_solve])
            - fluid_prop.compressibility
            * wcNplusHalf[to_solve]
            * pf_ch_prime
            * delta_tb[to_solve]
        )
        ...
        S[tip_indxs] = (
            ...
            + dt
            * (FinDiffOprtr[tip_indxs, :].tocsc()[:, tip_indxs]).dot(
                frac.pFluid[to_impose]
            )
            + dt
            * (FinDiffOprtr[tip_indxs, :].tocsc()[:, act_indxs]).dot(
                frac.pFluid[active]
            )
            + dt * G[to_impose]
            + dt * Q[to_impose] / frac.mesh.EltArea
            - LeakOff[to_impose] / frac.mesh.EltArea
            + -dt
            * (FinDiffOprtr[tip_indxs, :].tocsc()[:, ch_indxs]).dot(delta_tb[to_solve])
        )
        ...
        S[act_indxs] = (
            ...
            + dt
            * (FinDiffOprtr[act_indxs, :].tocsc()[:, tip_indxs]).dot(
                frac.pFluid[to_impose]
            )
            + dt
            * (FinDiffOprtr[act_indxs, :].tocsc()[:, act_indxs]).dot(
                frac.pFluid[active]
            )
            + dt * G[active]
            + dt * Q[active] / frac.mesh.EltArea
            - 0.0 * LeakOff[active] / frac.mesh.EltArea
            + -dt
            * (FinDiffOprtr[act_indxs, :].tocsc()[:, ch_indxs]).dot(delta_tb[to_solve])
        )
...
def EHL_dot(solk, args, wcNplusHalf, FinDiffOprtr, dtype=np.float64):
    ...
    res[ch_indxs] = (
        -(dt * FinDiffOprtr[ch_indxs, :].tocsc()[:, ch_indxs]).dot(res[ch_indxs])
        + fluid_prop.compressibility * wcNplusHalf[to_solve] * res[ch_indxs]
        + solk[ch_indxs]
    )

    # [2,1] INDEXES: act ch (generally)
    res[act_indxs] = -(dt * FinDiffOprtr[act_indxs, :].tocsc()[:, ch_indxs]).dot(
        c_dot_solk
    )

    # [3,1] INDEXES: tip ch (generally)
    res[tip_indxs] = -(dt * FinDiffOprtr[tip_indxs, :].tocsc()[:, ch_indxs]).dot(
        c_dot_solk
    )

    # [1,2] INDEXES: ch act (generally)
    res[ch_indxs] = res[ch_indxs] - dt * (
        FinDiffOprtr[ch_indxs, :].tocsc()[:, act_indxs]
    ).dot(solk[act_indxs])

    # [2,2] INDEXES: act act (generally)
    res[act_indxs] = (
        res[act_indxs]
        - dt * (FinDiffOprtr[act_indxs, :].tocsc()[:, act_indxs]).dot(solk[act_indxs])
        + fluid_prop.compressibility * wcNplusHalf[active] * solk[act_indxs]
    )

    # [3,2] INDEXES: tip act
    res[tip_indxs] = res[tip_indxs] - dt * (
        FinDiffOprtr[tip_indxs, :].tocsc()[:, act_indxs]
    ).dot(solk[act_indxs])

    # [1,3] INDEXES: ch tip (generally)
    res[ch_indxs] = res[ch_indxs] - dt * (
        FinDiffOprtr[ch_indxs, :].tocsc()[:, tip_indxs]
    ).dot(solk[tip_indxs])

    # [2,3] INDEXES: act tip
    res[act_indxs] = res[act_indxs] - dt * (
        FinDiffOprtr[act_indxs, :].tocsc()[:, tip_indxs]
    ).dot(solk[tip_indxs])

    # [3,3] INDEXES: tip tip
    res[tip_indxs] = (
        res[tip_indxs]
        - dt * (FinDiffOprtr[tip_indxs, :].tocsc()[:, tip_indxs]).dot(solk[tip_indxs])
...
def MakeEquationSystem_ViscousFluid_pressure_substituted(solk, interItr, *args):
    ...
    A[np.ix_(tip_indxs, ch_indxs)] = -dt * np.dot(
        FinDiffOprtr[np.ix_(tip_indxs, ch_indxs)], C[np.ix_(to_solve, to_solve)]
    )
    ...
    A[np.ix_(act_indxs, ch_indxs)] = -dt * np.dot(
        FinDiffOprtr[np.ix_(act_indxs, ch_indxs)], C[np.ix_(to_solve, to_solve)]
    )
...
def MakeEquationSystem_ViscousFluid_pressure_substituted_deltaP(solk, interItr, *args):
    ...
    S[tip_indxs] = (
        ...
        + dt * np.dot(FinDiffOprtr[np.ix_(tip_indxs, ch_indxs)], pf_ch_prime)
        + dt
        * np.dot(FinDiffOprtr[np.ix_(tip_indxs, tip_indxs)], frac.pFluid[to_impose])
    ...
    S[act_indxs] = (
        ...
        + dt * np.dot(FinDiffOprtr[np.ix_(act_indxs, ch_indxs)], pf_ch_prime)
        + dt
        * np.dot(FinDiffOprtr[np.ix_(act_indxs, tip_indxs)], frac.pFluid[to_impose])
...
def MakeEquationSystem_ViscousFluid_pressure_substituted_deltaP_injection(
    solk, interItr, *args
):
    """
    This function makes the linearized system of equations to be solved by a linear system solver. The system is
    ...
    A[np.ix_(ch_indxs, tip_indxs)] = -dt * FinDiffOprtr[np.ix_(ch_indxs, tip_indxs)]
    A[np.ix_(ch_indxs, act_indxs)] = -dt * FinDiffOprtr[np.ix_(ch_indxs, act_indxs)]
    A[ch_indxs[inj_in_ch], inj_ch_indx] -= (
        dt / frac.mesh.EltArea * np.ones(len(inj_ch_indx), dtype=np.float64)
    )
    ...
    A[np.ix_(act_indxs, act_indxs)] = -dt * FinDiffOprtr[np.ix_(act_indxs, act_indxs)]
    A[act_indxs, act_indxs] += fluid_prop.compressibility * wcNplusHalf[active]
    A[act_indxs[inj_in_act], inj_act_indx] -= (
        dt / frac.mesh.EltArea * np.ones(len(inj_act_indx), dtype=np.float64)
    )
    ...
    A[inj_act_indx, inj_act_indx] = -inj_prop.perforationFriction * abs(
        solk[inj_act_indx]
    )
...
# -----------------------------------------------------------------------------------------------------------------------
def MakeEquationSystem_ViscousFluid_pressure_substituted_deltaP_sparse_injection_line(
    solk, interItr, *args
):
    """
    This function makes the linearized system of equations to be solved by a linear system solver. The system is
    ...
    # from pyfrac.solid.elasticity_tip_correction import full_C_and_tip_correction_for_test
    #
    ...
    A[inj_act_indx, act_indxs[inj_in_act]] = -1.0
    A[inj_act_indx, p_il_indxs] = 1.0
    A[inj_act_indx, inj_act_indx] = -inj_prop.perforationFriction * abs(
        solk[inj_act_indx]
    )
    ...
    S = np.zeros((n_total,), dtype=np.float64)
    pf_ch_prime = (
        np.dot(C_to_solve_to_solve, frac.w[to_solve])
        + np.dot(C[np.ix_(to_solve, to_impose)], imposed_val)
        + np.dot(C[np.ix_(to_solve, active)], wNplusOne[active])
        + mat_prop.SigmaO[to_solve]
    )
...
def Elastohydrodynamic_ResidualFun_nd(
    solk, system_func, interItr, InterItr_o, indices_o, *args
):
    """
    This function gives the residual of the solution for the system of equations formed using the given function.
    """
    ...
    if len(indices[3]) == 0:
        ...
        Fx_red = (
            np.dot(A, np.delete(solk, len(indices[0]) + np.asarray(indices[3]))) - S
        )
...
def check_covergance(solk, solkm1, indices, tol):
    ...
    if w_normalization > 0.0:
        norm_w = np.linalg.norm(
            abs(solk[indices[0]] - solkm1[indices[0]]) / w_normalization
        )
    else:
        norm_w = np.linalg.norm(abs(solk[indices[0]] - solkm1[indices[0]]))

    p_normalization = np.linalg.norm(solkm1[indices[1]])
    if p_normalization > 0.0:
        norm_p = np.linalg.norm(
            abs(solk[indices[1]] - solkm1[indices[1]]) / p_normalization
        )
    else:
    ...
    if len(indices[2]) > 0:  # these are the cells with the active width constraints
        ...
        if tr_normalization > 0.0:
            norm_tr = np.linalg.norm(
                abs(solk[indices[2]] - solkm1[indices[2]]) / tr_normalization
            )
        else:
...
def get_complete_solution(sol, indices, *args):
    ...
    if sim_prop.solveDeltaP:
        values = (
            np.dot(C[np.ix_(tip_act[deleted], EltCrack)], w[EltCrack])
            + mat_prop.SigmaO[tip_act[deleted]]
            - frac.pFluid[tip_act[deleted]]
        )
    else:
        values = (
            np.dot(C[np.ix_(tip_act[deleted], EltCrack)], w[EltCrack])
...
def populate_full(indices, sol, values=None):
    ...
    sol_full[
        len(ch_indxs)
        + np.setdiff1d(np.arange(len(tip_indxs) + len(act_indxs)), deleted)
    ] = sol[len(ch_indxs) :]
