...
class Monolithic_EHL_sys_obj(LinearOperator):
    ...
    def _matvec_PastSolution(self, xks):
        ...
        :return: HMAT.v, where HMAT is a matrix obtained by selecting equations from either HMATtract or HMATdispl
        """
        return monolitic_EHL_dot_PastSolution(
            xks, self.args, self.wcNplusHalf, self.FinDiffOprtr
        )
    ...
    def _update_sys(self, solk, interItr):
        ...
        # compute RHS dw:
        active_and_toimpose = np.concatenate(
            (active, to_impose)
        )  # can be saved between iters
        C._set_domain_and_codomain_IDX(active_and_toimpose, to_solve)
        dw_active_and_tip = np.concatenate(
            ((wc_to_impose - frac.w[active]), (imposed_val - frac.w[to_impose]))
        )
        S[:n_ch] = -(C._matvec(dw_active_and_tip))

        # compute RHS dp:
        ch_act_toimpose = np.concatenate(
            (to_solve, active, to_impose)
        )  # can be saved between iters
        ...
        V_pn = np.dot(
            (
                FinDiffOprtr.tocsr()[np.arange(n_ch + n_act + n_tip), :].tocsc()[
                    :, np.arange(n_ch + n_act + n_tip)
                ]
            ).toarray(),
            frac.pFluid[ch_act_toimpose] - mat_prop.SigmaO[ch_act_toimpose],
        )
        # V_pn = np.dot(FinDiffOprtr.toarray()[:,:-1], frac.pFluid[ch_act_toimpose] - mat_prop.SigmaO[ch_act_toimpose])
        S[all_indxs_dp] = (
            dt * (-V_pn + G[ch_act_toimpose] + Q[ch_act_toimpose] / frac.mesh.EltArea)
...
def monolitic_EHL_dot(solk, args, wcNplusHalf, FinDiffOprtr, dtype=np.float64):
    ...
    res[w_ch_indxs] = (
        res[w_ch_indxs] + C._matvec(solk[p_ch_indxs]) / C.diag_val - solk[p_ch_indxs]
    )
    ...
    V_dp = dt * np.dot(
        (
            FinDiffOprtr.tocsr()[range(n_ch + n_act + n_tip), :].tocsc()[
                :, range(n_ch + n_act + n_tip)
            ]
        ).toarray(),
        solk[p_all_indxs],
    )
    ...
    res[p_all_indxs] = (
        ...
        * wcNplusHalf[np.concatenate((to_solve, active, to_impose))]
        * solk[p_all_indxs]
    )
...
def monolitic_EHL_dot_PastSolution(
    solk, args, wcNplusHalf, FinDiffOprtr, dtype=np.float64
):
    # todo: write the description of the arguments
    """
    Arguments:
    ...
    BT_S_z2 = (
        ...
        * np.dot(
            (
                FinDiffOprtr.tocsr()[range(n_ch), :].tocsc()[
                    :, range(n_ch + n_act + n_tip)
                ]
            ).toarray(),
            solk[p_all_indxs],
...
# -----------------------------------------------------------------------------------------------------------------------
#
def MakeEquationSystem_Monolithic_precond(
    ...
    S[ch_indxs_dw] = -np.dot(
        C[np.ix_(to_solve, active)], solk[2 * n_ch + np.arange(n_act)]
    ) + np.dot(
        C[np.ix_(to_solve, to_impose)], solk[2 * n_ch + n_act + np.arange(n_tip)]
    )
...
def makeA(
    ...
    # 12
    A[np.ix_(ch_indxs_dw, ch_indxs_dp)] = C[
        np.ix_(to_solve, to_solve)
    ] / C.diag_val - np.identity(n_ch)
    # 21
    A[np.ix_(ch_indxs_dp, ch_indxs_dw)] = (1.0 / C.diag_val) * np.identity(n_ch)
    # 22 (part)
    A[np.ix_(ch_indxs_dp, ch_indxs_dp)] = (-1.0 / C.diag_val) * np.identity(n_ch)
    # (2-4)(2-4)
    A[np.ix_(all_indxs_dp, all_indxs_dp)] = (
        A[np.ix_(all_indxs_dp, all_indxs_dp)]
        - dt
        * (
            FinDiffOprtr.tocsr()[np.arange(n_ch + n_act + n_tip), :].tocsc()[
