...
# Internal imports
from pyfrac.systems.make_sys_volume_and_load_control import (
    MakeEquationSystem_volumeControl,
    MakeEquationSystem_volumeControl_double_fracture,
    MakeEquationSystem_volumeControl_symmetric,
    Volume_Control_4_gmres,
)
from pyfrac.mesh_obj.symmetry import get_symetric_elements
from pyfrac.properties import instrument_start, instrument_close
from pyfrac.linear_solvers.linear_iterative_solver import iteration_counter


def sol_sys_volume_and_load_control(
    Fr_lstTmStp,
    sim_properties,
    fluid_properties,
    mat_properties,
    ...
    log = logging.getLogger(
        "PyFrac.solve_width_pressure.sol_sys_volume_and_load_control"
    )
    if sim_properties.volumeControlGMRES:
        ...
        total_vol = (
            sum(Fr_lstTmStp.w)
            + sum(Qin[EltCrack]) * (timeStep) / Fr_lstTmStp.mesh.EltArea
        )  # - something
        ...
        if (
            inj_same_footprint
        ):  # this is the case where you do not impose the tip opening

            S_i = -np.reciprocal(
                Fr_lstTmStp.EltCrack.size * D_i
            )  # Inverse Schur complement

            g1 = D_i * (mat_properties.SigmaO[Fr_lstTmStp.EltCrack]) + D_i * S_i * (
                total_vol
            ) * np.ones(
                Fr_lstTmStp.EltCrack.size
            )  # D_e^-1 * sigma - vol_incr * S^-1 * D_e^-1 *[1...1](vertical)
            ...
            S_i = -np.reciprocal(
                Fr_lstTmStp.EltChannel.size * D_i
            )  # Inverse Schur complement

            C._set_domain_and_codomain_IDX(EltTip, Fr_lstTmStp.EltChannel)
            g1 = D_i * (
                mat_properties.SigmaO[Fr_lstTmStp.EltChannel] - C._matvec(wTip)
            ) + D_i * S_i * (total_vol - np.sum(wTip)) * np.ones(
        ...
        sol_GMRES = gmres(
            system_dot_prod,
            rhs_prec,
            tol=sim_properties.gmres_tol,
            maxiter=sim_properties.gmres_maxiter,
            callback=counter,
        )
        ...
        if sol_GMRES[1] > 0:
            log.warning(
                "Volume Control system did NOT converge after "
                + str(sol_GMRES[1])
                + " iterations!"
            )
            rel_err = np.linalg.norm(
                system_dot_prod._matvec(sol_GMRES[0]) - (rhs_prec)
            ) / np.linalg.norm(rhs_prec)
            ...
            log.debug(
                " --> GMRES Volume Control converged after "
                + str(counter.niter)
                + " iter. & rel err is "
                + str(rel_err)
            )
        ...
        if sim_properties.symmetric and not sim_properties.useBlockToeplizCompression:
            try:
                ...
                raise SystemExit(
                    "Symmetric fracture needs symmetric mesh. Set symmetric flag to True\n"
            ...
            if (
                ...
                wtipindexFR0 = np.where(np.isin(EltTip, EltTipFracture0))[0]
                wtipindexFR1 = np.where(np.isin(EltTip, EltTipFracture1))[0]
                ...
                if np.any(
                    np.isin(
                        np.concatenate((EltChannelFracture0, EltChannelFracture1)),
                        np.concatenate((EltTipFracture0, EltTipFracture1)),
                        assume_unique=True,
                    )
                ):
                    SystemExit(
                ...
                A, b = MakeEquationSystem_volumeControl_double_fracture(
                    Fr_lstTmStp.w,
                    wTipFR0,
                    wTipFR1,
                    EltChannelFracture0,
                ...
                # time_beg=time.time()
                A, b = MakeEquationSystem_volumeControl(
                    Fr_lstTmStp.w,
                    wTip,
        ...
        if sim_properties.symmetric:
            ...
            for i in range(len(sol) - 1):
                del_w[
                    Fr_lstTmStp.mesh.symmetricElts[
                        Fr_lstTmStp.mesh.activeSymtrc[EltChannel_sym[i]]
                    ]
                ] = sol[i]
            w = np.copy(Fr_lstTmStp.w)
            ...
            if (
                ...
                w[EltChannelFracture1] += sol[
                    np.arange(
                        ...
                        EltChannelFracture0.size + EltChannelFracture1.size,
                    )
                ]
