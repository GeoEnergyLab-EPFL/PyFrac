...
def solve_width_pressure_RKL2(Eprime, GPU, n_threads, perf_node, *args):
    ...
    if turb:
        ...
        cond_0_lil = finiteDiff_operator_laminar(
            wLastTS,
            EltCrack,
            muPrime,
            Mesh.NeiElements,
    ...
    if gravity:
        ...
        G = Gravity_term(w_0, EltCrack, muPrime, Mesh, InCrack, rho)[EltCrack]
    ...
    pf_0[ch_indxs] = (
        np.dot(C[np.ix_(to_solve, EltCrack)], wLastTS[EltCrack]) + sigma0[to_solve]
    )
    pf_0[n_ch:] = np.linalg.solve(
        dt * mu_t_1 * (cond_0[n_ch:, n_ch:-1]).toarray(),
        act_tip_val
        - dt
        * mu_t_1
        * (
            cond_0[n_ch:, :][:, :n_ch].dot(pf_0[:n_ch])
            + G[n_ch:]
            + (Q[EltCrack[n_ch:]] - Lk_rate[EltCrack[n_ch:]]) / Mesh.EltArea
        ),
    )

    M_0 = (
        cond_0[:, :-1].dot(pf_0) + (Q[EltCrack] - Lk_rate[EltCrack]) / Mesh.EltArea + G
    )
...
def RKL_substep_neg(
    ...
    if turb:
        ...
        cond_lil = finiteDiff_operator_laminar(
            w_jm1,
            crack,
            muPrime,
            Mesh.NeiElements,
    ...
    gamma_t = -a[j - 1] * mu_t
    if gravity:
        G = Gravity_term(w_jm1, crack, muPrime, Mesh, InCrack, rho)[crack]
    else:
    ...
    M_jm1_tip = (
        np.dot((cond[n_channel:, :][:, :n_channel]).toarray(), pf[:n_channel])
        + G[n_channel:]
        + (Qin[n_channel:] - LeakOff[n_channel:]) / Mesh.EltArea
    )
    S = (
        imposed_value
        - mu[j] * W_jm1[n_channel:]
        - nu[j] * W_jm2[n_channel:]
        + (mu[j] + nu[j]) * W_0[n_channel:]
        - gamma_t * tau_M0[n_channel:]
        - mu_t * tau * M_jm1_tip
    )
...
def do_dot(a, b, out):
    ...
    out[:] = np.dot(
        a, b
    )  # less efficient because the output is stored in a temporary array?
...
def pardot_matrix_vector(a, b, nblocks, dot_func=do_dot):
    ...
    for i in range(nblocks):
        th = threading.Thread(
            target=dot_func, args=(a_blocks[i, :, :], b, out_blocks[i, :])
        )
        th.start()
        threads.append(th)
