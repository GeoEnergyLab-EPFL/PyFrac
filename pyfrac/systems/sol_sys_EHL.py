...
from pyfrac.systems.make_sys_back_subst_EHL import (
    MakeEquationSystem_ViscousFluid_pressure_substituted_deltaP_sparse,
    MakeEquationSystem_ViscousFluid_pressure_substituted_deltaP,
    EHL_sys_obj,
    MakeEquationSystem_ViscousFluid_pressure_substituted_sparse,
...
from pyfrac.systems.explicit_RKL import solve_width_pressure_RKL2
from pyfrac.systems.make_sys_common_fun import velocity
from pyfrac.systems.make_sys_monolithic_EHL import (
    Monolithic_EHL_sys_obj,
    MakeEquationSystem_Monolithic_precond,
)
from pyfrac.non_linear_solvers.picard_newton import Picard_Newton
from pyfrac.non_linear_solvers.anderson import Anderson
from pyfrac.properties import instrument_start, instrument_close
from pyfrac.linear_solvers.linear_direct_solver import Direct_linear_solver
from pyfrac.linear_solvers.linear_iterative_solver import Iterative_linear_solver
from pyfrac.linear_solvers.preconditioners.prec_back_subst_EHL import EHL_iLU_Prec
...
def sol_sys_EHL(
    ...
    if fluid_properties.turbulence:
        vk = np.zeros(
            (
                4,
                Fr_lstTmStp.mesh.NumberOfElts,
            ),
            dtype=np.float64,
        )
        wguess = np.copy(Fr_lstTmStp.w)
        if not inj_same_footprint:
            wguess[EltTip] = wTip
    ...
    if len(neg) > 0:
        ...
        wc_to_impose = np.array([], dtype=int)
    ...
    while active_contraint:
        ...
        EltCrack_k = np.concatenate((to_solve_k, neg), dtype=int)
        EltCrack_k = np.concatenate((EltCrack_k, to_impose_k), dtype=int)
        ...
        for i, elem in enumerate(EltCrack_k):
            corresponding = np.where(
                EltCrack_k == Fr_lstTmStp.mesh.NeiElements[elem, 0]
            )[0]
            if len(corresponding) > 0:
                corr_nei[i, 0] = corresponding[0]
            corresponding = np.where(
                EltCrack_k == Fr_lstTmStp.mesh.NeiElements[elem, 1]
            )[0]
            if len(corresponding) > 0:
                corr_nei[i, 1] = corresponding[0]
            corresponding = np.where(
                EltCrack_k == Fr_lstTmStp.mesh.NeiElements[elem, 2]
            )[0]
            if len(corresponding) > 0:
                corr_nei[i, 2] = corresponding[0]
            corresponding = np.where(
                EltCrack_k == Fr_lstTmStp.mesh.NeiElements[elem, 3]
            )[0]
            if len(corresponding) > 0:
                corr_nei[i, 3] = corresponding[0]

        lst_edgeInCrk = None
        if fluid_properties.rheology in ["Herschel-Bulkley", "HBF", "power law", "PLF"]:
            lst_edgeInCrk = [
                np.where(InCrack[Fr_lstTmStp.mesh.NeiElements[EltCrack_k, 0]])[0],
        ...
        avg_dw = (
            sum(Qin) * timeStep / Fr_lstTmStp.mesh.EltArea
            - sum(imposed_val_k - Fr_lstTmStp.w[to_impose_k])
        ) / len(to_solve_k)
        ...
        pf_guess_neg = (
            np.dot(C[np.ix_(neg, EltCrack_k)], w_guess[EltCrack_k])
            + mat_properties.SigmaO[neg]
        )
        pf_guess_tip = (
            np.dot(C[np.ix_(to_impose_k, EltCrack_k)], w_guess[EltCrack_k])
            + mat_properties.SigmaO[to_impose_k]
        )
        ...
        if (
            sim_properties.elastohydrSolver == "implicit_Picard"
            or sim_properties.elastohydrSolver == "implicit_Anderson"
        ):
            if inj_properties.modelInjLine:
                ...
                guess = np.concatenate(
                    (
                        np.full(len(to_solve_k), avg_dw, dtype=np.float64),
                ...
                if sim_properties.solveSparse:
                    if sim_properties.EHL_iter_lin_solve:
                        if not sim_properties.solve_monolithic:
                            ...
                            sys_fun = Monolithic_EHL_sys_obj(
                                len(to_solve_k) + sys_size, dtype=np.float64, *arg
                            )
                    else:
                        if not sim_properties.solve_monolithic:
                ...
                if not sim_properties.solve_monolithic:
                    ...
                    guess = np.concatenate(
                        (
                            np.full(len(to_solve_k), 0.0, dtype=np.float64),
                            pf_guess_neg - Fr_lstTmStp.pFluid[neg],
                            pf_guess_tip - Fr_lstTmStp.pFluid[to_impose_k],
                        )
                    )
            ...
            if inj_properties.modelInjLine:
                ...
                sink_cells = np.intersect1d(
                    inj_properties.sourceElem, Fr_lstTmStp.EltCrack
                )

                sink = np.zeros(Fr_lstTmStp.mesh.NumberOfElts)
                if inj_properties.sinkElem:
                    sink[inj_properties.sinkElem] = (
                        inj_properties.sinkVel * Fr_lstTmStp.mesh.EltArea
                    )

                indxCurTime = max(
                    np.where(
                        Fr_lstTmStp.time + timeStep
                        >= inj_properties.injectionRate[0, :]
                    )[0]
                )
                currentRate = inj_properties.injectionRate[
                    1, indxCurTime
                ...
                arg = (
                    ...
                    inj_act,
                    sink_cells,
                    Fr_lstTmStp.pInjLine,
                    np.asarray(inj_in_ch, dtype=int),
                    np.asarray(inj_in_act, dtype=int),
                    currentRate,
                    sink,
                )
            ...
            if sim_properties.EHL_iter_lin_solve:
                if not sim_properties.solve_monolithic:
                    if inj_same_footprint:
                        rcmp_prec_before2ndIter = True
                    else:
                        rcmp_prec_before2ndIter = True

                    linear_solver = Iterative_linear_solver(
                        sys_fun,
                        sim_properties.gmres_tol,
                        sim_properties.gmres_maxiter,
                        sim_properties.gmres_Restart,
            ...
            if sim_properties.elastohydrSolver == "implicit_Picard":
                ...
                sol, data_nonLinSolve = Picard_Newton(
                    None,
                    linear_solver,
                    guess,
                    typValue,
            ...
            sol, data_nonLinSolve = solve_width_pressure_RKL2(
        ...
        if perfNode_widthConstrItr is not None:
            ...
            if failed_sol:
                if (
                    len(perfNode_widthConstrItr.linearSolve_data)
        ...
        if len(below_wc_k) > 0:
            ...
            wHst_above_wc = np.where(
                Fr_lstTmStp.wHist[to_solve_k] >= mat_properties.wc
            )[0]
            ...
            wHst_below_wc = np.where(Fr_lstTmStp.wHist[to_solve_k] < mat_properties.wc)[
                0
            ]
        ...
        if sim_properties.solve_monolithic and sim_properties.solveDeltaP:
            ...
            pf_k[ch_act_toimpose] = (
                Fr_lstTmStp.pFluid[ch_act_toimpose] + sol[len(to_solve_k) :]
            )
            ...
            if sim_properties.useBlockToeplizCompression:
                ...
                pf_k[to_solve_k] = (
                    C._matvec(w[EltCrack]) + mat_properties.SigmaO[to_solve_k]
                )

            else:
                # Fluid pressure in the channel evaluated by dot product between elasticity matrix and w
                pf_k[to_solve_k] = (
                    np.dot(C[np.ix_(to_solve_k, EltCrack)], w[EltCrack])
                    + mat_properties.SigmaO[to_solve_k]
                )

            # Effect of the finite domain
            if Boundary is not None:
                pf_k[to_solve_k] = pf_k[to_solve_k] + Boundary.last_traction[to_solve_k]

        if sim_properties.solveDeltaP:
            pf_k[neg_km1] = (
                Fr_lstTmStp.pFluid[neg_km1]
                + sol[len(to_solve_k) : len(to_solve_k) + len(neg_km1)]
            )
            pf_k[to_impose_k] = (
                Fr_lstTmStp.pFluid[to_impose_k]
                + sol[
                    len(to_solve_k)
                    + len(neg_km1) : len(to_solve_k)
                    + len(neg_km1)
                    + len(to_impose_k)
                ]
            )
    ...
    if perfNode_nonLinSys is not None:
        instrument_close(
            perfNode, perfNode_nonLinSys, None, len(sol), True, None, Fr_lstTmStp.time
        )
        perfNode.nonLinSolve_data.append(perfNode_nonLinSys)
    ...
    if inj_properties.modelInjLine:
        ...
        Q_act = sol[
            pil_indx + 1 + len(inj_ch) : pil_indx + 1 + len(inj_ch) + len(inj_act)
        ]
    else:
        dp_il = None
        Q_ch = None
