...
# Internal imports
from pyfrac.fluid.fluid_model import friction_factor_vector, friction_factor_MDR


# @jit()
def finiteDiff_operator_laminar(
    w, EltCrack, muPrime, NeiElements, dx, dy, InCrack, neiInCrack, sparse_flag=True
):
    """
    ...
    if sparse_flag:
        FinDiffOprtr = sparse.lil_matrix(
            (len(EltCrack), len(EltCrack) + 1), dtype=np.float64
        )
    else:
    ...
    wLftEdge = (
        (w[EltCrack] + w[NeiElements[EltCrack, 0]])
        / 2
        * InCrack[NeiElements[EltCrack, 0]]
    )
    wRgtEdge = (
        (w[EltCrack] + w[NeiElements[EltCrack, 1]])
        / 2
        * InCrack[NeiElements[EltCrack, 1]]
    )
    wBtmEdge = (
        (w[EltCrack] + w[NeiElements[EltCrack, 2]])
        / 2
        * InCrack[NeiElements[EltCrack, 2]]
    )
    wTopEdge = (
        (w[EltCrack] + w[NeiElements[EltCrack, 3]])
        / 2
        * InCrack[NeiElements[EltCrack, 3]]
    )

    indx_elts = np.arange(len(EltCrack))
    FinDiffOprtr[indx_elts, indx_elts] = (
        -(wLftEdge**3 + wRgtEdge**3) / dx**2 - (wBtmEdge**3 + wTopEdge**3) / dy**2
    ) / muPrime
...
def Gravity_term(w, EltCrack, fluidProp, mesh, InCrack, simProp):
    ...
    if simProp.gravity:
        if fluidProp.rheology == "Newtonian" and not fluidProp.turbulence:
            # width at the cell edges evaluated by averaging. Zero if the edge is outside fracture
            wBtmEdge = (
                (w[EltCrack] + w[mesh.NeiElements[EltCrack, 2]])
                / 2
                * InCrack[mesh.NeiElements[EltCrack, 2]]
            )
            wTopEdge = (
                (w[EltCrack] + w[mesh.NeiElements[EltCrack, 3]])
                / 2
                * InCrack[mesh.NeiElements[EltCrack, 3]]
            )

            G[EltCrack] = (
                fluidProp.density
                * simProp.gravityValue
                * (wTopEdge**3 - wBtmEdge**3)
                / mesh.hy
                / fluidProp.muPrime
            )
        else:
            raise SystemExit(
                "Effect of gravity is only supported for Newtonian fluid in laminar flow regime yet!"
            )
...
def FiniteDiff_operator_turbulent_implicit(
    w,
    pf,
    EltCrack,
    fluidProp,
    ...
    dp[4, EltCrack] = (
        dp[2, mesh.NeiElements[EltCrack, 0]]
        + dp[3, mesh.NeiElements[EltCrack, 0]]
        + dp[2, EltCrack]
        + dp[3, EltCrack]
    ) / 4
    dp[5, EltCrack] = (
        dp[2, mesh.NeiElements[EltCrack, 1]]
        + dp[3, mesh.NeiElements[EltCrack, 1]]
        + dp[2, EltCrack]
        + dp[3, EltCrack]
    ) / 4
    dp[6, EltCrack] = (
        dp[0, mesh.NeiElements[EltCrack, 2]]
        + dp[1, mesh.NeiElements[EltCrack, 2]]
        + dp[0, EltCrack]
        + dp[1, EltCrack]
    ) / 4
    dp[7, EltCrack] = (
        dp[0, mesh.NeiElements[EltCrack, 3]]
        + dp[1, mesh.NeiElements[EltCrack, 3]]
        + dp[0, EltCrack]
        + dp[1, EltCrack]
    ) / 4
    ...
    for i in range(0, len(EltCrack)):
        ...
        if dpLft[i] < 1e-8 or wLftEdge[i] < 1e-10:
            ...
            if (
                Velocity_Residual(np.finfo(float).eps * vkm1[0, EltCrack[i]], *arg)
                * Velocity_Residual(upBracket_factor * vkm1[0, EltCrack[i]], *arg)
                > 0.0
            ):
                ...
                vk[0, EltCrack[i]] = brentq(
                    Velocity_Residual,
                    np.finfo(float).eps * vkm1[0, EltCrack[i]],
                    upBracket_factor * vkm1[0, EltCrack[i]],
                    arg,
                )
        ...
        if dpRgt[i] < 1e-8 or wRgtEdge[i] < 1e-10:
            ...
            # check if bracket gives residuals with opposite signs
            if (
                Velocity_Residual(np.finfo(float).eps * vkm1[1, EltCrack[i]], *arg)
                * Velocity_Residual(upBracket_factor * vkm1[1, EltCrack[i]], *arg)
                > 0.0
            ):
                # bracket not valid. finding suitable bracket
                ...
                # find the root with brentq method.
                vk[1, EltCrack[i]] = brentq(
                    Velocity_Residual,
                    np.finfo(float).eps * vkm1[1, EltCrack[i]],
                    upBracket_factor * vkm1[1, EltCrack[i]],
                    arg,
                )
        ...
        if dpBtm[i] < 1e-8 or wBtmEdge[i] < 1e-10:
            ...
            # check if bracket gives residuals with opposite signs
            if (
                Velocity_Residual(np.finfo(float).eps * vkm1[2, EltCrack[i]], *arg)
                * Velocity_Residual(upBracket_factor * vkm1[2, EltCrack[i]], *arg)
                > 0.0
            ):
                # bracket not valid. finding suitable bracket
                ...
                # find the root with brentq method.
                vk[2, EltCrack[i]] = brentq(
                    Velocity_Residual,
                    np.finfo(float).eps * vkm1[2, EltCrack[i]],
                    upBracket_factor * vkm1[2, EltCrack[i]],
                    arg,
                )
        ...
        if dpTop[i] < 1e-8 or wTopEdge[i] < 1e-10:
            ...
            # check if bracket gives residuals with opposite signs
            if (
                Velocity_Residual(np.finfo(float).eps * vkm1[3, EltCrack[i]], *arg)
                * Velocity_Residual(upBracket_factor * vkm1[3, EltCrack[i]], *arg)
                > 0.0
            ):
                # bracket not valid. finding suitable bracket
                ...
                # find the root with brentq method.
                vk[3, EltCrack[i]] = brentq(
                    Velocity_Residual,
                    np.finfo(float).eps * vkm1[3, EltCrack[i]],
                    upBracket_factor * vkm1[3, EltCrack[i]],
                    arg,
                )

    # calculating Reynold's number with the velocity
    ReLftEdge = (
        4 / 3 * fluidProp.density * wLftEdge * vk[0, EltCrack] / fluidProp.viscosity
    )
    ReRgtEdge = (
        4 / 3 * fluidProp.density * wRgtEdge * vk[1, EltCrack] / fluidProp.viscosity
    )
    ReBtmEdge = (
        4 / 3 * fluidProp.density * wBtmEdge * vk[2, EltCrack] / fluidProp.viscosity
    )
    ReTopEdge = (
        4 / 3 * fluidProp.density * wTopEdge * vk[3, EltCrack] / fluidProp.viscosity
    )

    # non zeros Reynolds numbers
    ReLftEdge_nonZero = np.where(ReLftEdge > 0.0)[0]
    ReRgtEdge_nonZero = np.where(ReRgtEdge > 0.0)[0]
    ReBtmEdge_nonZero = np.where(ReBtmEdge > 0.0)[0]
    ReTopEdge_nonZero = np.where(ReTopEdge > 0.0)[0]
    ...
    ffLftEdge[ReLftEdge_nonZero] = friction_factor_vector(
        ReLftEdge[ReLftEdge_nonZero], rough[ReLftEdge_nonZero]
    )
    ffRgtEdge[ReRgtEdge_nonZero] = friction_factor_vector(
        ReRgtEdge[ReRgtEdge_nonZero], rough[ReRgtEdge_nonZero]
    )
    ffBtmEdge[ReBtmEdge_nonZero] = friction_factor_vector(
        ReBtmEdge[ReBtmEdge_nonZero], rough[ReBtmEdge_nonZero]
    )
    ffTopEdge[ReTopEdge_nonZero] = friction_factor_vector(
        ReTopEdge[ReTopEdge_nonZero], rough[ReTopEdge_nonZero]
    )

    # the conductivity matrix
    cond = np.zeros((4, EltCrack.size), dtype=np.float64)
    cond[0, ReLftEdge_nonZero] = wLftEdge[ReLftEdge_nonZero] ** 2 / (
        fluidProp.density
        * ffLftEdge[ReLftEdge_nonZero]
        * vk[0, EltCrack[ReLftEdge_nonZero]]
    )
    cond[1, ReRgtEdge_nonZero] = wRgtEdge[ReRgtEdge_nonZero] ** 2 / (
        fluidProp.density
        * ffRgtEdge[ReRgtEdge_nonZero]
        * vk[1, EltCrack[ReRgtEdge_nonZero]]
    )
    cond[2, ReBtmEdge_nonZero] = wBtmEdge[ReBtmEdge_nonZero] ** 2 / (
        fluidProp.density
        * ffBtmEdge[ReBtmEdge_nonZero]
        * vk[2, EltCrack[ReBtmEdge_nonZero]]
    )
    cond[3, ReTopEdge_nonZero] = wTopEdge[ReTopEdge_nonZero] ** 2 / (
        fluidProp.density
        * ffTopEdge[ReTopEdge_nonZero]
        * vk[3, EltCrack[ReTopEdge_nonZero]]
    )

    # assembling the finite difference matrix
    FinDiffOprtr[EltCrack, EltCrack] = (
        -(cond[0, :] + cond[1, :]) / dx**2 - (cond[2, :] + cond[3, :]) / dy**2
    )
    ...
    FD_compressed[np.ix_(indx_elts, ch_indxs)] = FinDiffOprtr[
        np.ix_(EltCrack, to_solve)
    ]
    FD_compressed[np.ix_(indx_elts, act_indxs)] = FinDiffOprtr[np.ix_(EltCrack, active)]
    FD_compressed[np.ix_(indx_elts, tip_indxs)] = FinDiffOprtr[
        np.ix_(EltCrack, to_impose)
    ]
...
def finiteDiff_operator_power_law(
    w, pf, EltCrack, fluidProp, Mesh, InCrack, neiInCrack, edgeInCrk_lst, simProp
):
    """
    The function evaluate the finite difference 5 point stencil matrix, i.e. the A matrix in the ElastoHydrodynamic
    ...
    cond = np.zeros((4, EltCrack.size), dtype=np.float64)
    cond[0, edgeInCrk_lst[0]] = (
        wLftEdge[edgeInCrk_lst[0]] ** (2 * fluidProp.n + 1)
        * dpLft[edgeInCrk_lst[0]]
        / fluidProp.Mprime
    ) ** (1 / fluidProp.n) / dpLft[edgeInCrk_lst[0]]
    cond[1, edgeInCrk_lst[1]] = (
        wRgtEdge[edgeInCrk_lst[1]] ** (2 * fluidProp.n + 1)
        * dpRgt[edgeInCrk_lst[1]]
        / fluidProp.Mprime
    ) ** (1 / fluidProp.n) / dpRgt[edgeInCrk_lst[1]]
    cond[2, edgeInCrk_lst[2]] = (
        wBtmEdge[edgeInCrk_lst[2]] ** (2 * fluidProp.n + 1)
        * dpBtm[edgeInCrk_lst[2]]
        / fluidProp.Mprime
    ) ** (1 / fluidProp.n) / dpBtm[edgeInCrk_lst[2]]
    cond[3, edgeInCrk_lst[3]] = (
        wTopEdge[edgeInCrk_lst[3]] ** (2 * fluidProp.n + 1)
        * dpTop[edgeInCrk_lst[3]]
        / fluidProp.Mprime
    ) ** (1 / fluidProp.n) / dpTop[edgeInCrk_lst[3]]
...
def finiteDiff_operator_Herschel_Bulkley(
    w, pf, EltCrack, fluidProp, Mesh, InCrack, neiInCrack, edgeInCrk_lst, simProp
):
    """
    The function evaluate the finite difference 5 point stencil matrix, i.e. the A matrix in the ElastoHydrodynamic
    ...
    x0 = np.maximum(
        1 - 2 * fluidProp.T0 / wLftEdge / dpLft,
        np.zeros(len(wLftEdge), dtype=np.float64),
    )
    cond[0, edgeInCrk_lst[0]] = (
        fluidProp.var1
        * dpLft[edgeInCrk_lst[0]] ** fluidProp.var2
        * wLftEdge[edgeInCrk_lst[0]] ** fluidProp.var3
        * x0[edgeInCrk_lst[0]] ** fluidProp.var4
        * (
            1
            + 2
            * fluidProp.T0
            / wLftEdge[edgeInCrk_lst[0]]
            / dpLft[edgeInCrk_lst[0]]
            * fluidProp.var5
        )
    )
    x1 = np.maximum(
        1 - 2 * fluidProp.T0 / wRgtEdge / dpRgt,
        np.zeros(len(wLftEdge), dtype=np.float64),
    )
    cond[1, edgeInCrk_lst[1]] = (
        fluidProp.var1
        * dpRgt[edgeInCrk_lst[1]] ** fluidProp.var2
        * wRgtEdge[edgeInCrk_lst[1]] ** fluidProp.var3
        * x1[edgeInCrk_lst[1]] ** fluidProp.var4
        * (
            1
            + 2
            * fluidProp.T0
            / wRgtEdge[edgeInCrk_lst[1]]
            / dpRgt[edgeInCrk_lst[1]]
            * fluidProp.var5
        )
    )
    x2 = np.maximum(
        1 - 2 * fluidProp.T0 / wBtmEdge / dpBtm,
        np.zeros(len(wLftEdge), dtype=np.float64),
    )
    cond[2, edgeInCrk_lst[2]] = (
        fluidProp.var1
        * dpBtm[edgeInCrk_lst[2]] ** fluidProp.var2
        * wBtmEdge[edgeInCrk_lst[2]] ** fluidProp.var3
        * x2[edgeInCrk_lst[2]] ** fluidProp.var4
        * (
            1
            + 2
            * fluidProp.T0
            / wBtmEdge[edgeInCrk_lst[2]]
            / dpBtm[edgeInCrk_lst[2]]
            * fluidProp.var5
        )
    )
    x3 = np.maximum(
        1 - 2 * fluidProp.T0 / wTopEdge / dpTop,
        np.zeros(len(wLftEdge), dtype=np.float64),
    )
    cond[3, edgeInCrk_lst[3]] = (
        fluidProp.var1
        * dpTop[edgeInCrk_lst[3]] ** fluidProp.var2
        * wTopEdge[edgeInCrk_lst[3]] ** fluidProp.var3
        * x3[edgeInCrk_lst[3]] ** fluidProp.var4
        * (
            1
            + 2
            * fluidProp.T0
            / wTopEdge[edgeInCrk_lst[3]]
            / dpTop[edgeInCrk_lst[3]]
            * fluidProp.var5
        )
    )
    ...
    if simProp.saveEffVisc:
        with np.errstate(divide="ignore"):
            eff_mu = np.zeros((4, Mesh.NumberOfElts), dtype=np.float64)
            eff_mu[0, EltCrack[edgeInCrk_lst[0]]] = wLftEdge[edgeInCrk_lst[0]] ** 3 / (
                12 * cond[0, edgeInCrk_lst[0]]
            )
            eff_mu[1, EltCrack[edgeInCrk_lst[1]]] = wRgtEdge[edgeInCrk_lst[1]] ** 3 / (
                12 * cond[1, edgeInCrk_lst[1]]
            )
            eff_mu[2, EltCrack[edgeInCrk_lst[2]]] = wBtmEdge[edgeInCrk_lst[2]] ** 3 / (
                12 * cond[2, edgeInCrk_lst[2]]
            )
            eff_mu[3, EltCrack[edgeInCrk_lst[3]]] = wTopEdge[edgeInCrk_lst[3]] ** 3 / (
                12 * cond[3, edgeInCrk_lst[3]]
            )
...
def get_finite_difference_matrix(
    ...
    if fluid_prop.rheology == "Newtonian" and not fluid_prop.turbulence:
        FinDiffOprtr = finiteDiff_operator_laminar(
            wNplusOne,
            EltCrack,
            fluid_prop.muPrime,
            mesh.NeiElements,
        ...
        pf[to_solve] = (
            np.dot(C[np.ix_(to_solve, EltCrack)], wNplusOne[EltCrack])
            + mat_prop.SigmaO[to_solve]
        )
        if sim_prop.solveDeltaP:
            pf[active] = (
                frac_n.pFluid[active] + sol[len(to_solve) : len(to_solve) + len(active)]
            )
            # to implement injection line
            # pf[to_impose] = frac_n.pFluid[to_impose] + sol[len(to_solve) + len(active):]
            pf[to_impose] = (
                frac_n.pFluid[to_impose]
                + sol[
                    len(to_solve)
                    + len(active) : len(to_solve)
                    + len(active)
                    + len(to_impose)
                ]
            )
        ...
        if fluid_prop.turbulence:
            FinDiffOprtr, interItr_kp1[0] = FiniteDiff_operator_turbulent_implicit(
                wNplusOne,
                pf,
                EltCrack,
                fluid_prop,
            ...
            FinDiffOprtr, interItr_kp1[2], interItr_kp1[3] = (
                finiteDiff_operator_Herschel_Bulkley(
                    wNplusOne,
                    pf,
                    EltCrack,
                    fluid_prop,
            ...
            FinDiffOprtr, interItr_kp1[2] = finiteDiff_operator_power_law(
                wNplusOne,
                pf,
                EltCrack,
                fluid_prop,
...
def velocity(w, EltCrack, Mesh, InCrack, muPrime, C, sigma0):
    ...
    """
    dpdxLft, dpdxRgt, dpdyBtm, dpdyTop = pressure_gradient(
        w, C, sigma0, Mesh, EltCrack, InCrack
    )
    ...
    vel[0, EltCrack] = (
        -(((w[EltCrack] + w[Mesh.NeiElements[EltCrack, 0]]) / 2) ** 2)
        / muPrime[EltCrack]
        * dpdxLft
    )
    vel[1, EltCrack] = (
        -(((w[EltCrack] + w[Mesh.NeiElements[EltCrack, 1]]) / 2) ** 2)
        / muPrime[EltCrack]
        * dpdxRgt
    )
    vel[2, EltCrack] = (
        -(((w[EltCrack] + w[Mesh.NeiElements[EltCrack, 2]]) / 2) ** 2)
        / muPrime[EltCrack]
        * dpdyBtm
    )
    vel[3, EltCrack] = (
        -(((w[EltCrack] + w[Mesh.NeiElements[EltCrack, 3]]) / 2) ** 2)
        / muPrime[EltCrack]
        * dpdyTop
    )

    vel[4, EltCrack] = (
        vel[2, Mesh.NeiElements[EltCrack, 0]]
        + vel[3, Mesh.NeiElements[EltCrack, 0]]
        + vel[2, EltCrack]
        + vel[3, EltCrack]
    ) / 4
    vel[5, EltCrack] = (
        vel[2, Mesh.NeiElements[EltCrack, 1]]
        + vel[3, Mesh.NeiElements[EltCrack, 1]]
        + vel[2, EltCrack]
        + vel[3, EltCrack]
    ) / 4
    vel[6, EltCrack] = (
        vel[0, Mesh.NeiElements[EltCrack, 2]]
        + vel[1, Mesh.NeiElements[EltCrack, 2]]
        + vel[0, EltCrack]
        + vel[1, EltCrack]
    ) / 4
    vel[7, EltCrack] = (
        vel[0, Mesh.NeiElements[EltCrack, 3]]
        + vel[1, Mesh.NeiElements[EltCrack, 3]]
        + vel[0, EltCrack]
        + vel[1, EltCrack]
    ) / 4
...
def pressure_gradient(w, C, sigma0, Mesh, EltCrack, InCrack):
    ...
    dpdxLft = (pf[EltCrack] - pf[Mesh.NeiElements[EltCrack, 0]]) * InCrack[
        Mesh.NeiElements[EltCrack, 0]
    ]
    dpdxRgt = (pf[Mesh.NeiElements[EltCrack, 1]] - pf[EltCrack]) * InCrack[
        Mesh.NeiElements[EltCrack, 1]
    ]
    dpdyBtm = (pf[EltCrack] - pf[Mesh.NeiElements[EltCrack, 2]]) * InCrack[
        Mesh.NeiElements[EltCrack, 2]
    ]
    dpdyTop = (pf[Mesh.NeiElements[EltCrack, 3]] - pf[EltCrack]) * InCrack[
        Mesh.NeiElements[EltCrack, 3]
    ]
...
def pressure_gradient_form_pressure(pf, Mesh, EltCrack, InCrack):
    ...
    dpdxLft = (
        (pf[EltCrack] - pf[Mesh.NeiElements[EltCrack, 0]])
        * InCrack[Mesh.NeiElements[EltCrack, 0]]
        / Mesh.hx
    )
    dpdxRgt = (
        (pf[Mesh.NeiElements[EltCrack, 1]] - pf[EltCrack])
        * InCrack[Mesh.NeiElements[EltCrack, 1]]
        / Mesh.hx
    )
    dpdyBtm = (
        (pf[EltCrack] - pf[Mesh.NeiElements[EltCrack, 2]])
        * InCrack[Mesh.NeiElements[EltCrack, 2]]
        / Mesh.hy
    )
    dpdyTop = (
        (pf[Mesh.NeiElements[EltCrack, 3]] - pf[EltCrack])
        * InCrack[Mesh.NeiElements[EltCrack, 3]]
        / Mesh.hy
    )
...
def calculate_fluid_flow_characteristics_laminar(
    w, pf, sigma0, Mesh, EltCrack, InCrack, muPrime, density, simulProp, solid
):
    """
    ...
    if muPrime != 0:
        dp = np.zeros((8, Mesh.NumberOfElts), dtype=np.float64)
        dpdxLft, dpdxRgt, dpdyBtm, dpdyTop = pressure_gradient_form_pressure(
            pf, Mesh, EltCrack, InCrack
        )
        ...
        # width at the cell edges evaluated by averaging. Zero if the edge is outside fracture
        wLftEdge = (
            (w[EltCrack] + w[Mesh.NeiElements[EltCrack, 0]])
            / 2
            * InCrack[Mesh.NeiElements[EltCrack, 0]]
        )
        wRgtEdge = (
            (w[EltCrack] + w[Mesh.NeiElements[EltCrack, 1]])
            / 2
            * InCrack[Mesh.NeiElements[EltCrack, 1]]
        )
        wBtmEdge = (
        ...
        fluid_flux = np.vstack(
            (-(wLftEdge**3) * dpLft / muPrime, -(wRgtEdge**3) * dpRgt / muPrime)
        )
        ...
        fluid_flux_components = np.vstack(
            (
                -(wLftEdge**3) * dp[0, EltCrack] / muPrime,
                -(wLftEdge**3) * dp[4, EltCrack] / muPrime,
            )
        )
        #                                                      fx right edge
        fluid_flux_components = np.vstack(
            (fluid_flux_components, -(wRgtEdge**3) * dp[1, EltCrack] / muPrime)
        )
        #                                                      fy right edge
        fluid_flux_components = np.vstack(
            (fluid_flux_components, -(wRgtEdge**3) * dp[5, EltCrack] / muPrime)
        )
        #                                                      fx bottom edge
        fluid_flux_components = np.vstack(
            (fluid_flux_components, -(wBtmEdge**3) * dp[6, EltCrack] / muPrime)
        )
        #                                                      fy bottom edge
        fluid_flux_components = np.vstack(
            (fluid_flux_components, -(wBtmEdge**3) * dp[2, EltCrack] / muPrime)
        )
        #                                                      fx top edge
        fluid_flux_components = np.vstack(
            (fluid_flux_components, -(wTopEdge**3) * dp[7, EltCrack] / muPrime)
        )
        #                                                      fy top edge
        fluid_flux_components = np.vstack(
            (fluid_flux_components, -(wTopEdge**3) * dp[3, EltCrack] / muPrime)
        )

        if simulProp.gravity:
            # we average the density between neighboring cells
            rhoLftEdge = (
                solid.density[EltCrack] + solid.density[Mesh.NeiElements[EltCrack, 0]]
            ) / 2
            rhoRgtEdge = (
                solid.density[EltCrack] + solid.density[Mesh.NeiElements[EltCrack, 1]]
            ) / 2
            rhoBtmEdge = (
                solid.density[EltCrack] + solid.density[Mesh.NeiElements[EltCrack, 2]]
            ) / 2
            rhoTopEdge = (
                solid.density[EltCrack] + solid.density[Mesh.NeiElements[EltCrack, 3]]
            ) / 2
        ...
        for i in range(8):
            ...
            fluid_vel_components[i][local_nonzero_indexes] /= wEdges[
                int(np.trunc(i / 2))
            ][local_nonzero_indexes]
        ...
        return (
            ...
            fluid_vel_components,
        )
    else:
        raise SystemExit(
            "ERROR: if the fluid viscosity is equal to 0 does not make sense to compute the fluid velocity or the fluid flux"
        )
