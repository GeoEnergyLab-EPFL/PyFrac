...
# Internal imports
from pyfrac.properties import LabelProperties, IterationProperties, PlotProperties
from pyfrac.properties import instrument_start, instrument_close

from pyfrac.solid.elasticity_Transv_Isotropic import (
    load_TI_elasticity_matrix,
    load_TI_elasticity_matrix_toepliz,
)
from pyfrac.solid.elasticity_kernels.isotropic_R0_elem import (
    load_isotropic_elasticity_matrix,
)
from pyfrac.solid.elasticity_isotropic import load_isotropic_elasticity_matrix_toepliz
from pyfrac.solid.elasticity_isotropic_symmetric import (
    load_isotropic_elasticity_matrix_symmetric,
    symmetric_elasticity_matrix_from_full,
)

from pyfrac.mesh_obj.mesh import CartesianMesh
from pyfrac.mesh_obj.remesh import *

from pyfrac.time_step.ts_solution import attempt_time_step

from pyfrac.utilities.visualization import (
    plot_footprint_analytical,
    plot_analytical_solution,
    plot_injection_source,
    get_elements,
    EPFLcolor,
)
from pyfrac.utilities.labels import (
    TS_errorMessages,
    supported_projections,
    suitable_elements,
)

from pyfrac.custom_functions import *
...
class Controller:
    ...
    def __init__(
        ...
        self.fr_queue = [
            ...
            None,
            None,
        ]  # queue of fractures from the last five time steps
        self.stepsFromChckPnt = 0
        self.tmStpPrefactor_copy = copy.copy(
            Sim_prop.tmStpPrefactor
        )  # should be in simulation properties
        ...
        )  # copy of the flag indicating the solver to solve for pressure or delta p
        ...
        self.fullyClosed = (
            False  # should be related to the fracture state (thus in fracture class)
        )
        self.setFigPos = True
        ...
        if "custom" in self.sim_prop.plotVar:
            if hasattr(self.sim_prop.custom, "number_of_plots"):
                self.number_of_figures = (
                    len(self.sim_prop.plotVar)
                    - 1
                    + self.sim_prop.custom.number_of_plots
                )
            else:
                message = (
                    "CUSTOM CLASS ERROR: \n"
        ...
        if Injection_prop.injectionRate.shape[1] > 1:
            param_change_at = np.hstack(
                (param_change_at, Injection_prop.injectionRate[0])
            )
        ...
        if len(param_change_at) > 0:
            if self.sim_prop.get_solTimeSeries() is not None:
                ...
                sol_time_srs = np.hstack(
                    (self.sim_prop.get_solTimeSeries(), param_change_at)
                )
            else:
        ...
        if self.sim_prop.finalTime is None:
            if self.sim_prop.get_solTimeSeries() is None:
                ...
                raise ValueError(
                    "The final time to stop the simulation is not provided!"
                )
            else:
                self.sim_prop.finalTime = np.max(self.sim_prop.get_solTimeSeries())
        else:
            if self.timeToHit is not None:
                greater_finalTime = np.where(self.timeToHit > self.sim_prop.finalTime)[
                    0
                ]
        ...
        if not all(
            elem in self.fracture.EltChannel for elem in Injection_prop.sourceElem
        ):
            message = (
                "INJECTION LOCATION ERROR: \n"
        ...
        if self.fluid_prop.rheology in ["Herschel-Bulkley", "HBF"]:
            if self.sim_prop.get_tipAsymptote() not in [
                "HBF",
                "HBF_aprox",
                "HBF_num_quad",
            ]:
                warnings.warn(
                    "Fluid rhelogy and tip asymptote does not match. Setting tip asymptote to 'HBF'"
                )
        ...
        if self.fluid_prop.rheology in ["power-law", "PLF"]:
            if self.sim_prop.get_tipAsymptote() not in [
                ...
                warnings.warn(
                    "Fluid rhelogy and tip asymptote does not match. Setting tip asymptote to 'PLF'"
                )
        ...
        if self.fluid_prop.rheology == "Newtonian":
            if self.sim_prop.get_tipAsymptote() not in [
                ...
                warnings.warn(
                    "Fluid rhelogy and tip asymptote does not match. Setting tip asymptote to 'U'"
                )
                self.sim_prop.set_tipAsymptote("U1")

        if Solid_prop.inv_with_heter_K1c and (
            Sim_prop.frontAdvancing == "explicit"
            or Sim_prop.frontAdvancing == "predictor-corrector"
        ):
            warnings.warn(
                "Heterogeneous_K1c may be not accurate for explicit or predictor-corrector"
            )

        # if you set the code to advance max 1 cell then remove the SimulProp.timeStepLimit
        if (
            self.sim_prop.timeStepLimit is not None
            and self.sim_prop.limitAdancementTo2cells is True
        ):
            if self.sim_prop.forceTmStpLmtANDLmtAdvTo2cells == False:
                warnings.warn(
                    "You have set sim_prop.limitAdancementTo2cells = True. This imply that sim_prop.timeStepLimit will be deactivated."
                )
    ...
    def run(self):
        ...
        ):  # in case you save 1/4 of the elasticity due to domain symmetry
        ...
        if self.C is None:
            ...
            if not self.solid_prop.TI_elasticity:
                if self.sim_prop.symmetric:
                    log.info(
                        "Making elasticity using load_isotropic_elasticity_matrix_symmetric"
                    )
                    ...
                    if not self.sim_prop.useBlockToeplizCompression:
                        log.info(
                            "Making elasticity using load_isotropic_elasticity_matrix"
                        )
                        ...
                        # # Bigwham hmat
                        # conn = self.fracture.mesh.Connectivity
                        # coor = self.fracture.mesh.VertexCoor
                        # coor = np.hstack([coor, np.zeros((coor.shape[0], 1))])
                        # elas_prop = np.array([self.solid_prop.Eprime, 0.0])
                        # kernel_elt = "3DR0-H-mode1"
                        # eta = 4.0
                        # max_leaf_size = 32
                        # eps_aca = 1.e-4
                        # self.C = BEMatrix(kernel_elt, coor, conn, elas_prop, max_leaf_size, eta, eps_aca,n_openMP_threads=8)

                        # print("Bigwham Hmat built")

                    else:
                        log.info(
                            "Making elasticity using load_isotropic_elasticity_matrix_toepliz"
                        )
                        self.C = load_isotropic_elasticity_matrix_toepliz(
                            self.fracture.mesh,
                            self.solid_prop.Eprime,
                            Kernel=self.sim_prop.elasticKernel,
                            useHMATdot=self.sim_prop.useHmat,
                        )
            else:
                if not self.sim_prop.useBlockToeplizCompression:
                    log.info("Making elasticity using load_TI_elasticity_matrix")
                    C = load_TI_elasticity_matrix(
                        self.fracture.mesh.Lx,
                    ...
                    log.info(
                        "Making elasticity using load_TI_elasticity_matrix_toepliz"
                    )
                    self.C = load_TI_elasticity_matrix_toepliz(
                        self.fracture.mesh,
                        self.solid_prop.Cij,
                        self.sim_prop.TI_KernelExecPath,
                    )
        ...
        if self.sim_prop.send_phone_msg:
            from pyfrac.utilities.utility import send_phone_message
        ...
        while (
            self.sim_prop.terminating_criterion(self.fracture)
            and self.TmStpCount < self.sim_prop.maxTimeSteps
        ):

            log.info(f"TIMESTEP {self.TmStpCount}")
            ...
            while adaptive_time_refinement:
                ...
                status, Fr_n_pls1 = self.advance_time_step(
                    self.fracture, self.C, timeStep_i, tmStp_perf
                )
                ...
                if status == 1:
                    timeStep_i, adaptive_time_refinement = (
                        self.sim_prop.adaptive_time_refinement(
                            self.fracture, Fr_n_pls1, timeStep_i
                        )
                    )
                else:
            ...
            if status == 1:
                ...
                log.debug(
                    f"for a time step of: {Fr_n_pls1.time - self.fracture.time} s"
                )
                ...
                if self.sim_prop.customPlotsOnTheFly:
                    Fr_n_pls1 = self.sim_prop.custom.postprocess_fracture(
                        self.sim_prop,
                        self.solid_prop,
                        self.fluid_prop,
                        self.injection_prop,
                ...
                if self.sim_prop.enableRemeshing:
                    ...
                    if status == 16:
                        ...
                        new_elems = [
                            int(
                                (
                                    self.fracture.mesh.nx
                                    + np.round(self.sim_prop.meshReductionFactor, 0)
                                )
                        ...
                        if (
                            2 * self.fracture.mesh.Lx / new_elems[0]
                            > self.sim_prop.maxCellSize
                        ) or (
                            2 * self.fracture.mesh.Ly / new_elems[1]
                            > self.fracture.mesh.hy
                            / self.fracture.mesh.hx
                            * self.sim_prop.maxCellSize
                        ):
                            log.warning(
                                "Reduction of cells not possible as minimal cell size would be violated!"
                            )
                            ...
                            if len(self.fracture.source) == 1:
                                ...
                                cent_point = self.fracture.mesh.CenterCoor[
                                    self.fracture.source[0]
                                ]
                                ...
                                if (
                                    self.fracture.mesh.CenterCoor[
                                        self.fracture.source[0]
                                    ][0]
                                    == self.fracture.mesh.CenterCoor[
                                        self.fracture.source[1]
                                    ][0]
                                ):
                                    elems_inter = int(
                                        abs(
                                ...
                                log.info(
                                    "The real reduction factor used is "
                                    + repr(reduction_factor)
                                )

                            else:
                                index = self.fracture.mesh.locate_element(0.0, 0.0)[0]
                                cent_point = np.asarray([0.0, 0.0])
                            ...
                            new_limits = [
                                [
                                    cent_point[0]
                                    - round((new_elems[0] - 1) / (1 / col_frac + 1))
                                    * self.fracture.mesh.hx
                                    * reduction_factor,
                                    cent_point[0]
                                    + (
                                ...
                                [
                                    cent_point[1]
                                    - round((new_elems[1] - 1) / (row_frac + 1))
                                    * self.fracture.mesh.hy
                                    * reduction_factor,
                                    cent_point[1]
                                    + (
                            ...
                            if np.min(
                                np.sqrt(
                                    (
                                        self.fracture.Ffront[::, [0, 2]].flatten()
                                        - cent_point[0]
                                    )
                                    ** 2
                                    + (
                                        self.fracture.Ffront[::, [1, 3]].flatten()
                                        - cent_point[1]
                                    )
                                    ** 2
                                )
                                ...
                                np.abs(
                                    [
                                        (new_limits[0][0] - new_limits[0][1])
                                        / elems[0],
                                        (new_limits[1][0] - new_limits[1][1])
                                        / elems[1],
                                    ]
                                )
                            ):
                        ...
                        front_indices = np.intersect1d(
                            self.fracture.mesh.Frontlist,
                            Fr_n_pls1.EltTip,
                            return_indices=True,
                        )[1]
                        side_bools = [
                            (front_indices <= Fr_n_pls1.mesh.nx - 3).any(),
                            (
                                front_indices[front_indices > Fr_n_pls1.mesh.nx - 3]
                                <= 2 * (Fr_n_pls1.mesh.nx - 3) + 1
                            ).any(),
                            (
                                front_indices[
                                    front_indices >= 2 * (Fr_n_pls1.mesh.nx - 2)
                                ]
                                % 2
                                == 0
                            ).any(),
                        ...
                        if not self.sim_prop.meshExtensionAllDir:
                            compress = not np.asarray(
                                np.asarray(self.sim_prop.meshExtension)
                                * np.asarray(side_bools)
                            ).any() or (
                                len(
                                    np.asarray(side_bools)[
                                        np.asarray(side_bools) == True
                                    ]
                                )
                                > 3
                            )
                    ...
                    if compress:
                        ...
                        if len(self.fracture.source) == 1:
                            ...
                            if (
                                self.fracture.mesh.CenterCoor[self.fracture.source[0]][
                                    0
                                ]
                                == self.fracture.mesh.CenterCoor[
                                    self.fracture.source[1]
                                ...
                                new_inter = int(
                                    np.ceil(elems_inter / self.sim_prop.remeshFactor)
                                )
                        ...
                        new_limits = [
                            [
                                cent_point[0]
                                - round(
                                    (self.fracture.mesh.nx - 1) / (1 / col_frac + 1)
                                )
                                * self.fracture.mesh.hx
                                * compression_factor,
                                cent_point[0]
                            ...
                            [
                                cent_point[1]
                                - round((self.fracture.mesh.ny - 1) / (row_frac + 1))
                                * self.fracture.mesh.hy
                                * compression_factor,
                                cent_point[1]
                                + (
                                    self.fracture.mesh.ny
                                    - round(
                                        (self.fracture.mesh.ny - 1) / (row_frac + 1)
                                    )
                                    - 1
                                )
                                * self.fracture.mesh.hy
                        ...
                        if (
                            len(np.intersect1d(self.fracture.mesh.CenterElts, index))
                            == 0
                        ):
                        ...
                        if np.min(
                            ...
                            np.abs(
                                [
                                    (new_limits[0][0] - new_limits[0][1]) / elems[0],
                            ...
                            self.remesh(
                                new_limits, elems, rem_factor=compression_factor
                            )

                        side_bools = [False, False, False, False]

                    else:
                        # --- This is the mesh extension --- #

                        # We get the new limits
                        new_limits, elems, direction = get_extensionsal_domain_limits(
                            np.asarray(
                                np.asarray(self.sim_prop.meshExtension)
                                * np.asarray(side_bools)
                            ),
                    ...
                    if np.asarray(side_bools).any():
                        ...
                        if len(self.fracture.source) == 1:
                            ...
                            index = self.fracture.mesh.locate_element(0.0, 0.0)
                            cent_point = np.asarray([0.0, 0.0])
                ...
                if self.PstvInjJmp:
                    ...
                    time_larger = np.where(
                        Fr_n_pls1.time <= self.injection_prop.injectionRate[0, :]
                    )[0]
                    pos_inj = np.where(self.injection_prop.injectionRate[1, :] > 0)[0]
                    Qact = self.injection_prop.get_injection_rate(
                        self.fracture.time, self.fracture
                    )
                    after_time = np.intersect1d(time_larger, pos_inj)
                    if len(after_time) == 0 and max(Qact) == 0.0:
                        ...
                        jump_to = min(
                            self.injection_prop.injectionRate[
                ...
                log.info(
                    "The fracture is advancing more than two cells in a row at time "
                ...
                if self.TmStpReductions == self.sim_prop.maxReattemptsFracAdvMore2Cells:
                    ...
                    if self.sim_prop.collectPerfData:
                        if self.sim_prop.saveToDisk:
                            file_address = (
                                self.sim_prop.get_outputFolder() + "perf_data.dat"
                            )
                        else:
                    ...
                    if self.sim_prop.send_phone_msg:
                        ...
                        send_phone_message(
                            "---Simulation " + self.sim_prop.simID + " failed---"
                        )
                    ...
                    if isinstance(self.sim_prop.tmStpPrefactor, np.ndarray):
                        indxCurTime = max(
                            np.where(
                                self.fracture.time >= self.sim_prop.tmStpPrefactor[0, :]
                            )[0]
                        )
                        self.sim_prop.tmStpPrefactor[1, indxCurTime] *= (
                            0.5**self.TmStpReductions
                        )
                    else:
                ...
                if (
                    self.fr_queue[self.successfulTimeSteps % 5] is None
                    or self.chkPntReattmpts == 4
                ):
                    if self.sim_prop.collectPerfData:
                        if self.sim_prop.saveToDisk:
                            file_address = (
                    ...
                    self.solid_prop.remesh(
                        self.fr_queue[
                            (self.successfulTimeSteps + self.chkPntReattmpts) % 5
                        ].mesh
                    )
                    self.injection_prop.remesh(
                        self.fr_queue[
                            (self.successfulTimeSteps + self.chkPntReattmpts) % 5
                        ].mesh,
                        self.fracture.mesh,
        ...
        for i in range(0, self.sim_prop.maxReattempts):
            if status not in [18]:
                ...
                if i > self.sim_prop.maxReattempts / 2 - 1:
                    tmStp_to_attempt = timeStep * (
                        1 / self.sim_prop.reAttemptFactor
                    ) ** (i + 1 - self.sim_prop.maxReattempts / 2)
            else:
            ...
            print("\n")
            log.info(
                f"Evaluating solution at time = {Frac.time+tmStp_to_attempt:.4g} s ..."
            )
    ...
    def output(self, Fr_advanced):
        ...
        if (
            self.sim_prop.get_solTimeSeries() is not None
            and self.sim_prop.plotATsolTimeSeries
        ):
        ...
        if self.sim_prop.saveToDisk:
            ...
            if self.sim_prop.saveTimePeriod is not None:
                if (
                    Fr_advanced.time
                    >= self.lastSavedTime + self.sim_prop.saveTimePeriod
                ):
        ...
        if self.sim_prop.plotFigure:
            ...
            if plot_TP_exceeded or in_req_TSrs or plot_TS_exceeded:
                for index_pltvar, plt_var in enumerate(self.sim_prop.plotVar):
                    ...
                    if (
                        plt_var != "custom"
                    ):  # assuming custom the last in sim_prop.plotVar
                        index = index_pltvar
                        if self.Figures[index]:
                            axes = self.Figures[
                                index
                            ].get_axes()  # save axes from last figure
                            plt.figure(self.Figures[index].number)
                            plt.clf()  # clear figure
                            self.Figures[index].add_axes(
                                axes[0]
                    ...
                    if plt_var == "footprint":
                        ...
                        if self.sim_prop.plotAnalytical:
                            self.Figures[index] = plot_footprint_analytical(
                                self.sim_prop.analyticalSol,
                                self.solid_prop,
                                self.injection_prop,
                                self.fluid_prop,
                                [Fr_advanced.time],
                        ...
                        self.Figures[index] = Fr_advanced.plot_fracture(
                            variable="mesh",
                        ...
                        self.Figures[index] = plot_injection_source(
                            self.fracture, fig=self.Figures[index]
                        )
                        ...
                        for counter_local_in_custom_plot, index_i in enumerate(index):
                            if self.Figures[index_i]:
                                ...
                                plt.clf()  # clear figure
                                self.Figures[index_i].add_axes(
                                    axes[0]
                                )  # add axis to the figure

                            self.Figures[index_i] = self.sim_prop.custom.custom_plot(
                                counter_local_in_custom_plot + 1,
                                self.sim_prop,
                                fig=self.Figures[index_i],
                            )
                            plt.ion()
                            plt.pause(0.0001)
                    elif plt_var in (
                        "fluid velocity as vector field",
                        "fvvf",
                        "fluid flux as vector field",
                        "ffvf",
                    ):
                        if self.fluid_prop.viscosity == 0.0:
                            raise SystemExit(
                                "ERROR: if the fluid viscosity is equal to 0 does not make sense to ask a plot of the fluid velocity or fluid flux"
                            )
                        elif self.sim_prop._SimulationProperties__tipAsymptote == "K":
                            raise SystemExit(
                                "ERROR: if tipAsymptote == K, does not make sense to ask a plot of the fluid velocity or fluid flux"
                            )
                        self.Figures[index] = Fr_advanced.plot_fracture(
                            variable="mesh",
                            mat_properties=self.solid_prop,
                        ...
                        if self.sim_prop.plotAnalytical:
                            ...
                            self.Figures[index] = plot_analytical_solution(
                                regime=self.sim_prop.analyticalSol,
                                variable=plt_var,
                        ...
                        self.Figures[index] = Fr_advanced.plot_fracture(
                            ...
                            elements=get_elements(
                                suitable_elements[plt_var], Fr_advanced
                            ),
                        )
                        # plotting source elements
                        self.Figures[index] = plot_injection_source(
                    ...
                    if len(Fr_advanced.closed) > 0 and plt_var != "custom":
                        ...
                        self.Figures[index] = Fr_advanced.mesh.identify_elements(
                            Fr_advanced.closed,
                            fig=self.Figures[index],
                            plot_prop=plot_prop,
                            plot_mesh=False,
    ...
    def get_time_step(self):
        ...
        time_step = 0
        if (
            self.sim_prop.fixedTmStp is not None
            and not self.sim_prop.force_time_schedule
        ):
            # fixed time step
            if isinstance(self.sim_prop.fixedTmStp, float) or isinstance(
                self.sim_prop.fixedTmStp, int
            ):
                time_step = self.sim_prop.fixedTmStp
                time_step_given = True
            elif (
                isinstance(self.sim_prop.fixedTmStp, np.ndarray)
                and self.sim_prop.fixedTmStp.shape[0] == 2
            ):
                # fixed time step list is given
                times_past = np.where(
                    self.fracture.time >= self.sim_prop.fixedTmStp[0, :]
                )[0]
                if len(times_past) > 0:
                ...
                raise ValueError(
                    "Fixed time step can be a float or an ndarray with two rows giving the time and"
        ...
        if not time_step_given and not self.sim_prop.force_time_schedule:
            ...
            if len(non_zero_v) > 0:
                if len(self.injection_prop.sourceElem) < 4:
                    # if point source
                    tipVrtxCoord = self.fracture.mesh.VertexCoor[
                        self.fracture.mesh.Connectivity[
                            self.fracture.EltTip, self.fracture.ZeroVertex
                        ]
                    ]
                    # the distance of tip from the injection point in each of the tip cell
                    dist_Inj_pnt = (
                        (tipVrtxCoord[:, 0] - self.injection_prop.sourceCoordinates[0])
                        ** 2
                        + (
                            tipVrtxCoord[:, 1]
                            - self.injection_prop.sourceCoordinates[1]
                        )
                        ** 2
                    ) ** 0.5 + self.fracture.l
                    ...
                    TS_fracture_length = min(
                        abs(
                            0.2 * dist_Inj_pnt[non_zero_v] / self.fracture.v[non_zero_v]
                        )
                    )
                else:
            ...
            # index of current time in the time series (first row) of the injection rate array
            indx_cur_time = max(
                np.where(self.fracture.time >= self.injection_prop.injectionRate[0, :])[
                    0
                ]
            )
            current_rate = self.injection_prop.injectionRate[
                1, indx_cur_time
            ]  # current injection rate
            if current_rate < 0:
                vel_injection = current_rate / (
                    2
                    * (self.fracture.mesh.hx + self.fracture.mesh.hy)
            ...
            current_prefactor = self.sim_prop.get_time_step_prefactor(
                self.fracture.time, self.fracture, estimated_ts
            )
            time_step = current_prefactor * min(
                TS_cell_length, TS_fracture_length, TS_inj_cell, TS_delta_vol
            )

            # limit time step to be max 2 * last time step
            if (
                self.lstTmStp != None and not np.isinf(time_step)
            ) and time_step > 2 * self.lstTmStp:
        ...
        if (
            time_step <= 0 or np.isinf(time_step)
        ) and not self.sim_prop.force_time_schedule:
            if self.stagnant_TS is not None:
                ...
                TS_obtained = False
                log.warning(
                    "The fracture front is stagnant and there is no injection. In these conditions, "
                    "there is no criterion to calculate time step size."
                )
                while not TS_obtained:
                    try:
                        inp = input(
                            "Enter the time step size(seconds) you would like to try:"
                        )
                        time_step = float(inp)
                        TS_obtained = True
                    except ValueError:
        ...
        if next_in_TS < self.fracture.time:
            raise SystemExit(
                "The minimum time required in the given time series or the end time"
                " is less than initial time."
            )

        # check if time step would step over the next time in required time series
        if (
            self.fracture.time + time_step > next_in_TS
        ) and not self.sim_prop.force_time_schedule:
        ...
        # very small time step in the next time step advance.
            ...
            next_in_TS - self.fracture.time < 1.05 * time_step
            and not self.sim_prop.force_time_schedule
        ):
            time_step = next_in_TS - self.fracture.time

        elif self.sim_prop.force_time_schedule:
            if np.abs(last_scheduled_in_TS - self.fracture.time) < np.maximum(
                last_step_scheduled / 1000.0, 1.0e-4
            ):
                time_step = next_in_TS - self.fracture.time

            elif (
                last_scheduled_in_TS < self.fracture.time
                and self.fracture.time < next_in_TS
            ):
                time_step = last_step_scheduled
                done_in_TS = self.timeToHit[
                    np.setdiff1d(np.arange(len(self.timeToHit)), larger_in_TS)
                ]
                shifted_TS = self.timeToHit[larger_in_TS] + (
                    self.fracture.time + last_step_scheduled - next_in_TS
                )
        ...
        if (
            self.sim_prop.timeStepLimit is not None
            and time_step > self.sim_prop.timeStepLimit
        ):
            log.warning(
                "Evaluated/given time step is more than the time step limit! Limiting time step..."
            )
    ...
    def extend_isotropic_elasticity_matrix(self, new_mesh, direction=None):
        ...
        new_indexes = np.array(
            mapping_old_indexes(new_mesh, self.fracture.mesh, direction)
        )

        if len(self.C) != Ne and not self.sim_prop.symmetric:
            self.C = np.vstack(
                (
                    np.hstack((self.C, np.full((Ne_old, Ne - Ne_old), 0.0))),
                    np.full((Ne - Ne_old, Ne), 0.0),
                )
            )

            self.C[np.ix_(new_indexes, new_indexes)] = self.C[
                np.ix_(np.arange(Ne_old), np.arange(Ne_old))
            ]
            ...
            for i in add_el:
                ...
                self.C[i] = (self.solid_prop.Eprime / (8.0 * np.pi)) * (
                    np.sqrt(np.square(a - x) + np.square(b - y)) / ((a - x) * (b - y))
                    + np.sqrt(np.square(a + x) + np.square(b - y)) / ((a + x) * (b - y))
                    + np.sqrt(np.square(a - x) + np.square(b + y)) / ((a - x) * (b + y))
                    + np.sqrt(np.square(a + x) + np.square(b + y)) / ((a + x) * (b + y))
                )

            self.C[np.ix_(new_indexes, add_el)] = np.transpose(
                self.C[np.ix_(add_el, new_indexes)]
            )
            ...
            self.C = load_isotropic_elasticity_matrix_symmetric(
                new_mesh, self.solid_prop.Eprime
            )
