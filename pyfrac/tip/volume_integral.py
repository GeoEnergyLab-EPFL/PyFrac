...
# Internal imports
# from pyfrac.tip.tip_inversion import f, C1, C2
...
def TipAsym_MK_W_zrthOrder_Res(w, *args):
    ...
    return w_tld - (
        1 + beta_m**3 * Eprime**2 * Vel * dist**0.5 * muPrime / Kprime**3
    ) ** (1 / 3)
...
def MomentsTipAssympGeneral(
    dist, Kprime, Eprime, muPrime, Cbar, Vel, stagnant, KIPrime, regime
):
    ...
    if dist == 0:
        ...
        try:
            if regime == "U":
                w = brentq(
                    TipAsym_UniversalW_zero_Res, a, b, TipAsmptargs
                )  # root finding
            else:
                w = brentq(
                    TipAsym_UniversalW_delt_Res, a, b, TipAsmptargs
                )  # root finding
        except RuntimeError:
            M0, M1 = np.nan, np.nan
            return M0, M1
...
def VolumeTriangle(dist, *param):
    ...
    if regime == "A":
        ...
        return (
            0.7081526678
            * (Vel * fluid_prop.muPrime / Eprime) ** (1 / 3)
            * em
            * dist ** (8 / 3)
        )
        ...
        return (
            256
            / 273
            / (15 * np.tan(np.pi / 8)) ** 0.25
            * (Cbar * fluid_prop.muPrime / Eprime) ** 0.25
            * em
            * Vel**0.125
            * dist ** (21 / 8)
        )
        ...
        return (
            ...
            * (
                ...
                * (
                    Kprime**6
                    - 1872.0 * dist * Eprime**4 * fluid_prop.muPrime**2 * Vel**2
                )
                + (
                    1.0
                    + (
                ...
                * (
                    ...
                    * Eprime**4
                    * Kprime**9
                    * fluid_prop.muPrime**2
                    * Vel**2
                    - 24624.0
                    * dist**1.5
                    * Eprime**6
                    * Kprime**6
                    * fluid_prop.muPrime**3
                    * Vel**3
                    + 464660.73424811783
                    * dist**2
                    * Eprime**8
                    * Kprime**3
                    * fluid_prop.muPrime**4
                    * Vel**4
                    + 5.7316896e7
                    * dist**2.5
                    * Eprime**10
                    * fluid_prop.muPrime**5
                    * Vel**5
                )
            )
        ) / (Eprime**11 * fluid_prop.muPrime**5 * Vel**5)
...
def Area(dist, *param):
    ...
    if regime == "A":
        ...
        return (
            7.348618459729571e-6
            * Kprime
            * (
                -1.7320508075688772 * Kprime**9
                + (
                    1.0
                    + (
                        31.17691453623979
                ...
                * (
                    ...
                    * Vel
                    ...
                    + (81648.0 * dist**1.5 * Eprime**6 * fluid_prop.muPrime**3 * Vel**3)
                )
            )
    ...
    while i < len(l):
        ...
        if abs(alpha[i]) < 1e-8:
            ...
            if l[i] <= mesh.hx:
                ...
                integral[i] = (
                    Area(l[i], *param_pack) - Area(l[i] - mesh.hx, *param_pack)
                ) * mesh.hy
            ...
            if l[i] <= mesh.hy:
                ...
                # the front has surpassed this cell.
                integral[i] = (
                    Area(l[i], *param_pack) - Area(l[i] - mesh.hy, *param_pack)
                ) * mesh.hx
        else:
            yIntrcpt = l[i] / np.cos(
                np.pi / 2 - alpha[i]
            ...
            if (
                lUp > 0
            ):  # upper vertex of the triangle is higher than the grid cell height
            ...
            if (
                lRt > 0
            ):  # right vertex of the triangle is wider than the grid cell width
        ...
        if (
            projMethod == "LS_continousfront"
            and function == "A"
            and integral[i] / mesh.EltArea > 1.0 + 1e-4
        ):
            log.debug(
...
def FindBracket_w_HB(a, b, *args):
    ...
    a = (
        ...
        * np.sqrt(dist)
        / Eprime
        * (1 + (np.sqrt(4 * np.pi * T0t) * xt) ** alpha) ** (1 / alpha)
...
def find_corresponding_ribbon_cell(tip_cells, alpha, zero_vertex, mesh):
    ...
    for i in range(len(tip_cells)):
        if alpha[i] == 0:
            ...
            if zero_vertex[i] == 0:
                corr_ribbon[i] = mesh.NeiElements[
                    mesh.NeiElements[tip_cells[i], 2], 0
                ]  # A
            elif zero_vertex[i] == 1:
                corr_ribbon[i] = mesh.NeiElements[
                    mesh.NeiElements[tip_cells[i], 2], 1
                ]  # G
            elif zero_vertex[i] == 2:
                corr_ribbon[i] = mesh.NeiElements[
                    mesh.NeiElements[tip_cells[i], 3], 1
                ]  # E
            elif zero_vertex[i] == 3:
                corr_ribbon[i] = mesh.NeiElements[
                    mesh.NeiElements[tip_cells[i], 3], 0
                ]  # C
...
def leak_off_stagnant_tip(
    Elts, l, alpha, vrtx_arr_time, current_time, Cprime, time_step, mesh
):
    """
    ...
    LkOff = (
        2 * Cprime[Elts] * (t_since_arrival**0.5 - t_since_arrival_lstTS**0.5) * area
    )
