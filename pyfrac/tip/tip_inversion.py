...
from pyfrac.mesh_obj.mesh import get_8neighbors
from pyfrac.level_set.level_set_utils import (
    get_LS_on_cell_vertexes,
    get_LSangle_on_cell_vertexes,
)
...
def TipAsym_m_exp(dist, *args):
    ...
    return -wEltRibbon + (V * fluidProp.muPrime / Eprime) ** (1 / 3) * dist ** (
        2 / 3
    ) * (
        beta_m
        + 1 / 2 * (l_mmt / dist) ** (1 / 6)
...
def TipAsym_mt_exp(dist, *args):
    ...
    return -wEltRibbon + (2 * Cbar * V ** (1 / 2) * fluidProp.muPrime / Eprime) ** (
        1 / 4
    ) * dist ** (5 / 8) * (
        0.0161 * (l_mtk / dist) ** (5 / 8 - 0.06999)
        + 2.53356
        + 1.30165 * (dist / l_mmt) ** (1 / 8)
...
def TipAsym_viscStor_Res(dist, *args):
    ...
    return wEltRibbon - (
        18 * 3**0.5 * (dist - DistLstTSEltRibbon) / dt * fluidProp.muPrime / Eprime
    ) ** (1 / 3) * dist ** (2 / 3)
...
def TipAsym_MDR_Res(dist, *args):
    ...
    return (
        ...
        - (
            1.89812
            * dist**0.740741
            * ((dist - DistLstTSEltRibbon) / dt) ** 0.481481
            * (fluidProp.muPrime**0.7 * density**0.3) ** 0.37037
        )
        / Eprime**0.37037
    )
...
def TipAsym_M_MDR_Res(dist, *args):
    ...
    return wEltRibbon - 3.14735 * dist ** (2 / 3) * (
        (dist - DistLstTSEltRibbon) / dt
    ) ** (1 / 3) * fluidProp.muPrime ** (1 / 3) * (
        1
        + 0.255286
        * dist**0.2
        * Vel**0.4
        * density**0.3
        / (Eprime**0.1 * fluidProp.muPrime**0.2)
    ) ** 0.37037 / Eprime ** (
        1 / 3
    )
...
def TipAsym_viscLeakOff_Res(dist, *args):
    ...
    return wEltRibbon - 4 / (15 * np.tan(np.pi / 8)) ** 0.25 * (
        2 * Cbar * fluidProp.muPrime / Eprime
    ) ** 0.25 * ((dist - DistLstTSEltRibbon) / dt) ** 0.125 * dist ** (5 / 8)
...
def TipAsym_MK_zrthOrder_Res(dist, *args):
    ...
    V = (dist - DistLstTSEltRibbon) / dt
    return w_tld - (
        1 + beta_m**3 * Eprime**2 * V * dist**0.5 * fluidProp.muPrime / Kprime**3
    ) ** (1 / 3)
...
def TipAsym_Universal_1stOrder_Res(dist, *args):
    ...
    if len(args) == 7:
        ...
        wEltRibbon, Kprime, Eprime, fluidProp, Cbar, DistLstTSEltRibbon, dt, i, mesh = (
            args  # heterog K
        )
        ...
        if isinstance(Kprime_val, (list, np.ndarray)):
            ...
            for i in range(len(Kprime_val)):
                ...
                if DistLstTSEltRibbon - dist[i] != 0.0:
                    args = (
                        wEltRibbon,
                        Kprime_val_i,
                        Eprime,
...
def TipAsym_Hershcel_Burkley_Res(dist, *args):
    ...
    wt = ((wEltRibbon * Eprime / Kprime / np.sqrt(dist)) ** alpha - wtTau**alpha) ** (
        1 / alpha
    )
    ...
    Bmt = (
        64 * (1 + n) ** 2 / (3 * n * (4 + n)) * np.tan(3 * np.pi * n / (4 * (1 + n)))
    ) ** (1 / (2 + 2 * n))
    ...
    dt1 = (
        ...
        * Vm
        * (
            Bm ** ((2 + n) / n) * Vmt ** ((1 + theta) / n)
            + X / wt * Bmt ** (2 * (1 + n) / n) * Vm ** ((1 + theta) / n)
        )
        / (
    ...
    return xt ** ((2 - n) / (1 + theta)) - dt1 * wt ** ((2 + n) / (1 + theta)) * (
        dm ** (1 + theta) * Bm ** (2 + n)
        + dmt ** (1 + theta) * Bmt ** (2 * (1 + n)) * ((1 + X / wt) ** n - 1)
    ) ** (-1 / (1 + theta))
...
def TipAsym_Hershcel_Burkley_MK_Res(dist, *args):
    ...
    return wt - (1 + (Bm ** (2 + n) * xt ** (2 - n)) ** (1 / (1 + theta))) ** (
        (1 + theta) / (2 + n)
    )
...
def TipAsym_PowerLaw_M_vertex_Res(dist, *args):
    ...
    return wEltRibbon - Bm * (Mprime * Vel**n / Eprime) ** (1 / (2 + n)) * dist ** (
        2 / (2 + n)
    )
...
def FindBracket_dist(
    w, Kprime, Eprime, fluidProp, Cprime, DistLstTS, dt, mesh, ResFunc, simProp
):
    """
    ...
    if fluidProp.rheology == "Newtonian" or sum(Cprime) == 0:
        ...
        for i in range(0, len(w)):
            ...
            b[i] = fsolve(
                Vm_residual, (w[i] * Eprime[i] / Kprime[i]) ** 2, args=TipAsmptargs
            )
    ...
    for i in range(0, len(w)):
        ...
        while Res_a * Res_b > 0:
            ...
            if (
                cnt >= 100
            ):  # Should assume not propagating. not set to check how frequently it happens.
                a[i] = np.nan
                b[i] = np.nan
                break
...
def bisection_Kjump_finder(a, b, Kprime, i, mesh):
    ...
    if delta_K_delta_s != 0.0:
        a_new = a
        b_new = b
        dist_th = mesh.hx / 1.0e-6  # hard threshold
        dist = b - a

        while dist > dist_th:
            midpoint = (a_new + b_new) * 0.5
...
def FindBracket_dist_K_heterog(
    w,
    Kprime,
    Eprime,
    fluidProp,
    ...
    if fluidProp.rheology == "Newtonian":
        a = -DistLstTS * (
            1 + 5e3 * np.finfo(float).eps
        )  # Small correction to make sure the distance is never 0!
        b = np.zeros(a.size)
        for i in range(0, len(w)):
            ai = a[i]
            grid1D = np.arange(ai, ai + maxprop, mesh.cellDiag / 20.0, dtype=np.float64)
            ...
            # Res(a) < 0  && Res(b) > 0
            test_Res_grid1D = Res_grid1D > 0.0
            if np.sum(test_Res_grid1D) == 0:
                ...
                if np.sum(np.diff(Res_grid1D) > 0.0) == 0:
                    log.debug(
                        "No bracket found because the initial guess propagates to far. Assume stagnant fracture!"
                    )
                    a[i] = np.nan
                    b[i] = np.nan
                else:
                    ...
                    log.debug(
                        "The front is advancing more than 8 cell diagonals --> trying to allow that"
                    )
                    ...
                    while np.sum(test_Res_grid1D) == 0:
                        # take a larger range
                        end_range = (
                            initial_prefactor * initial_prefactor * mesh.cellDiag
                        )
                        grid1D = np.arange(
                            ai, ai + end_range, mesh.cellDiag / 20.0, dtype=np.float64
                        )
                        ...
                        if not np.sum(test_Res_grid1D) == 0:
                            ...
                            if adv > 15:
                                log.info(
                                    "Failing the time step because the front is advancing more than 15 cell diagonals "
                                )
                                return a, b, 1
                        else:
                            prefactor = prefactor + 0.1
            ############################ END OF ADAPTION ############################
            elif np.sum(test_Res_grid1D) == len(
                test_Res_grid1D
            ):  # you have got all true
                ...
                Kjump_bool, a_new, b_new = bisection_Kjump_finder(
                    a[i], b[i], Kprime, i, mesh
                )
                if Kjump_bool:
                    ...
                    if f_a * f_a_new < 0.0:
                        b[i] = a_new
                    elif f_b * f_b_new < 0.0:
                        a[i] = b_new
                    elif f_a_new * f_b_new < 0.0:
                        a[i] = np.nan
                    else:
...
def TipAsymInversion(
    ...
    if matProp.inv_with_heter_K1c:
        stagnant = np.where(
            Kprime.of(frac.sgndDist[frac.EltRibbon])
            * (abs(frac.sgndDist[frac.EltRibbon])) ** 0.5
            / (Eprime * w[frac.EltRibbon])
            > 1.0
        )[0]
        # from utility import plot_as_matrix
    ...
    moving = np.arange(frac.EltRibbon.shape[0])[
        ~np.isin(frac.EltRibbon, frac.EltRibbon[stagnant])
    ]

    if matProp.inv_with_heter_K1c:  # and simParmtrs.get_volumeControl():
        Kprime.keepRibbonThatAre(moving)
        a, b, status = FindBracket_dist_K_heterog(
            w[frac.EltRibbon[moving]],
            Kprime,
            Eprime[moving],
            fluidProp,
    ...
    if any(np.isnan(a)):
        ...
        if not stagnant.size == 0:
            stagnant = np.sort(
                np.unique(np.hstack((stagnant, moving[stagnant_from_bracketing])))
            )
        else:
    ...
    for i in range(0, len(moving)):
        ...
        try:
            if perfNode is None:
                ...
                dist[moving[i]], data = brentq(
                    ResFunc, a[i], b[i], TipAsmptargs, full_output=True
                )
                instrument_close(
                    perfNode, brentq_itr, None, None, data.converged, None, None
                )
            ...
            if simParmtrs.get_tipAsymptote() == "U1":
                ...
                try:
                    if perfNode is None:
                        dist[moving[i]] = brentq(
                            TipAsym_Universal_zrthOrder_Res, a[i], b[i], TipAsmptargs
                        )
                    else:
                        brentq_itr = instrument_start("Brent method", perfNode)
                        dist[moving[i]], data = brentq(
...
def StressIntensityFactor(w, lvlSetData, EltTip, EltRibbon, stagnant, mesh, Eprime):
    ...
    for i in range(0, len(EltTip)):
        if stagnant[i]:
            enclosing = get_8neighbors(
                mesh.NeiElements, EltTip[i]
            )  # eight enclosing cells
            ...
            if InRibbon.size == 1:
                KIPrime[i] = (
                    w[InRibbon[0]] * Eprime[i] / (-lvlSetData[InRibbon[0]]) ** 0.5
                )
                ...
                log.warning(
                    "ribbon cells not found in enclosure of the tip, evaluating with the closest ribbon cell"
                )
                RibbonCellsDist = (
                    (mesh.CenterCoor[EltRibbon, 0] - mesh.CenterCoor[EltTip[i], 0]) ** 2
                    + (mesh.CenterCoor[EltRibbon, 1] - mesh.CenterCoor[EltTip[i], 1])
                    ** 2
                ) ** 0.5
...
def StressIntensityFactorFromVolume(
    w, lvlSetData, EltTip, EltRibbon, stagnant, mesh, Eprime
):
    """
    ...
    for i in range(NoRib):
        ...
        ls_on_cell_vertexes = get_LS_on_cell_vertexes(
            lvlSetData[enclosing], lvlSetData[cell_ID]
        )

        l_ind = np.argmin(ls_on_cell_vertexes)
        l = -ls_on_cell_vertexes[l_ind]
        lsAngle_on_cell_vertexes = get_LSangle_on_cell_vertexes(
            lvlSetData[enclosing], lvlSetData[cell_ID]
        )
        ...
        KIPrime_ribbon[i] = (
            w[cell_ID] * mesh.EltArea * 10.0**orderEprime
        ) / Integral_over_cell(
            EltTip,
            np.asarray([alpha]),
            [l],
...
def TipAsymInversion_hetrogenous_toughness(w, frac, mat_prop, level_set):
    ...
    for i in range(0, len(frac.EltRibbon)):
        if zero_vrtx[i] == 0:
            ...
            alpha[i] = np.arccos(
                (
                    dist[frac.EltRibbon[i]]
                    - dist[frac.mesh.NeiElements[frac.EltRibbon[i], 1]]
                )
                / frac.mesh.hx
            )
            ...
            alpha[i] = np.arccos(
                (
                    dist[frac.EltRibbon[i]]
                    - dist[frac.mesh.NeiElements[frac.EltRibbon[i], 0]]
                )
                / frac.mesh.hx
            )
        ...
        if (
            abs(
                dist[frac.mesh.NeiElements[frac.EltRibbon[i], 2]]
                / dist[frac.mesh.NeiElements[frac.EltRibbon[i], 3]]
                - 1
            )
            < 1e-7
        ):
    ...
    for i in range(0, len(frac.EltRibbon)):
        ...
        sample_lngths = np.linspace(
            4 * (frac.mesh.hx**2 + frac.mesh.hy**2) ** 0.5 / 16,
            4 * (frac.mesh.hx**2 + frac.mesh.hy**2) ** 0.5,
            16,
        )
        cnt = 0
        res_prdct = 0
        while res_prdct >= 0 and cnt < 16:
            res_prdct = residual_zero * TipAsym_variable_Toughness_Res(
                sample_lngths[cnt], *TipAsmptargs
            )
            cnt += 1
        ...
        try:
            sol[i] = brentq(
                TipAsym_variable_Toughness_Res, 0, upper_bracket, TipAsmptargs
            )
        except RuntimeError:
            sol[i] = np.nan
...
def find_zero_vertex(Elts, level_set, mesh):
    ...
    for i in range(0, len(Elts)):
        ...
        if (
            level_set[neighbors[i, 0]] <= level_set[neighbors[i, 1]]
            and level_set[neighbors[i, 2]] <= level_set[neighbors[i, 3]]
        ):
            zero_vertex[i] = 0
        elif (
            level_set[neighbors[i, 0]] > level_set[neighbors[i, 1]]
