...
# internal import
from pyfrac.systems.make_sys_back_subst_EHL import check_covergance
...
def Anderson(linear_solver, guess, interItr_init, sim_prop, *args, perf_node=None):
    ...
    xks = np.full((m_Anderson + 2, guess.size), 0.0)
    Fks = np.full((m_Anderson + 1, guess.size), 0.0)
    Gks = np.full((m_Anderson + 1, guess.size), 0.0)
    ...
    while not converged:
        ...
        try:
            ...
            if k >= m_Anderson:
                ...
                omega_s = np.linalg.lstsq(
                    np.transpose(Fks[:indFG, ::] - Fks[indFG, ::]),
                    -Fks[indFG, ::],
                    rcond=None,
                )[0]
                omega_s = np.asarray(list(omega_s) + [1.0 - sum(omega_s)])
            ...
            if k >= m_Anderson:
                xks[indX, ::] = (1 - relax) * np.sum(
                    np.transpose(np.multiply(np.transpose(xks[indexes, ::]), omega_s)),
                    axis=0,
                ) + relax * np.sum(
                    np.transpose(
                        np.multiply(
                ...
                xks[indX, ::] = (1 - relax) * np.sum(
                    np.transpose(np.multiply(np.transpose(xks[:indX, ::]), omega_s)),
                    axis=0,
                ) + relax * np.sum(
                    np.transpose(np.multiply(np.transpose(Gks[:indX, ::]), omega_s)),
                    axis=0,
                )
        ...
        converged, norm = check_covergance(
            xks[indX_before, ::],
            xks[indX, ::],
            linear_solver.indices,
        ...
        if k == sim_prop.maxSolverItrs:  # returns nan as solution if does not converge
            log.warning(
                "Anderson iteration not converged after "
                + repr(sim_prop.maxSolverItrs)
                + " iterations, norm:"
                + repr(norm)
            )
