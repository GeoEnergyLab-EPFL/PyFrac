...
from pyfrac.systems.make_sys_back_subst_EHL import (
    check_covergance,
    Elastohydrodynamic_ResidualFun,
)
    ...
    *args,
    ...
    while not converged:  # todo:check system change (AM)
        ...
        if (k + 1) % PicardPerNewton == 0:
            Fx, interItr, indices = Elastohydrodynamic_ResidualFun(
                solk, linear_solver.sys_func, interItr, *args
            )
            Jac = Jacobian(
                Elastohydrodynamic_ResidualFun,
                linear_solver.sys_func,
                solk,
                TypValue,
                interItr,
                *args
        ...
        if k == sim_prop.maxSolverItrs:  # returns nan as solution if does not converge
            log.warning(
                "Picard iteration not converged after "
...
def Jacobian(Residual_function, sys_func, x, TypValue, interItr, *args):
    ...
    for i in range(0, len(x)):
        ...
        if central:
            ...
            xin[i] = xin[i] - Epsilon
            Jac[:, i] = (
                Residual_function(xip, sys_func, interItr, *args)[0]
                - Residual_function(xin, sys_func, interItr, *args)[0]
            ) / (2 * Epsilon)
