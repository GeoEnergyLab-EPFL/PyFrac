...
# internal imports
from pyfrac.level_set.discontinuous_front_reconstruction import reconstruct_front, UpdateLists
from pyfrac.level_set.FMM import fmm
from pyfrac.level_set.continuous_front_reconstruction import reconstruct_front_continuous, UpdateListsFromContinuousFrontRec, \
    get_xy_from_Ffront, get_cells_in_neighborhood, \
    ray_tracing_numpy
from pyfrac.solid.elasticity_tip_correction import tip_correction_

from pyfrac.tip.volume_integral import Integral_over_cell
from pyfrac.solid.elasticity_isotropic_symmetric import self_influence
...
def generate_footprint(mesh, surv_cells, inner_region, dist_surv_cells, projMethod):
    ...
    Returns:
        ...
        - sgndDist (ndarray-float)    -- signed minimun distance from pyfrac.fracture front of each cell in the domain.
    """
    ...
    # np.save("dist_surv_cells", dist_surv_cells)
    # np.save("surv_cells", surv_cells)
    # np.save("inner_region", inner_region)
    ...
    newTip_indices = np.arange(len(EltTip_tmp))[np.isin(EltTip_tmp, EltTip)]
