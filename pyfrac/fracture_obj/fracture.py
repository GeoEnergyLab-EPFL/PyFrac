...
from pyfrac.mesh_obj.remesh import mapping_old_indexes

from pyfrac.tip.volume_integral import find_corresponding_ribbon_cell, Pdistance, leak_off_stagnant_tip
from pyfrac.tip.tip_inversion import StressIntensityFactor

from pyfrac.level_set.anisotropy import *
from pyfrac.level_set.continuous_front_reconstruction import UpdateListsFromContinuousFrontRec
from pyfrac.level_set.level_set_utils import get_front_region
from pyfrac.level_set.discontinuous_front_reconstruction import UpdateLists
from pyfrac.level_set.FMM import fmm

from pyfrac.fracture_obj.fracture_initialization import get_survey_points, get_width_pressure, generate_footprint, \
    generate_footprint_from_Ffront
from pyfrac.fracture_obj.fracture_initialization import Geometry, InitializationParameters

from pyfrac.HF_reference_solutions import HF_analytical_sol

from pyfrac.utilities.visualization import plot_fracture_list, plot_fracture_list_slice, to_precision, zoom_factory, EPFLcolor
from pyfrac.utilities.labels import unidimensional_variables
...
class Fracture:
    ...
    Attributes:
        ...
        sgndDist (ndarray):         -- signed minimum distance from pyfrac.fracture front of each cell in the domain
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    def remesh(self, factor, C, coarse_mesh, material_prop, fluid_prop, inj_prop, sim_prop, direction):
        ...
        # if direction == None or direction == 'reduce':
            ...
            ### --- We evaluate now the level set from the griddata interpolation everywhere on the coarse mesh --- ###
            ...
            EltRibbon = np.delete(Fr_coarse.EltRibbon, np.where(sgndDist_last_coarse[Fr_coarse.EltRibbon] == np.inf)[0])
            ...
            # We evaluate the velocity fromt the two interpolated solutions (this is preciser than using the new object
            ...
            Fr_coarse.closed = np.asarray([], dtype=int)
...
# -----------------------------------------------------------------------------------------------------------------------
    def project_solution_to_a_new_mesh(self, C, newMesh, Solid, Fluid, Injection, simulProp, oldfront):
        ...
        # from pyfrac.level_set.continuous_front_reconstruction import plot_xy_points, get_xy_from_Ffront
...
# -----------------------------------------------------------------------------------------------------------------------
    def update_tip_regime(self, mat_prop, fluid_prop, timeStep):
        ...
        moving = np.arange(self.EltRibbon.shape[0])[~np.isin(self.EltRibbon, self.EltRibbon[stagnant])]
