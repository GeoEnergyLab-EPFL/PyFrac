...
if run:
    ...
    def K1c_func(x,y,alpha):
        ...
        r = 1.48
        delta = 0.0005
        return smoothing(K_Ic, 1.1*K_Ic, r, delta, x)
    ...
    simulProp.finalTime = 0.904612  # the time at which the simulation stops
...
if not os.path.isfile('./batch_run.txt'):  # We only visualize for runs of specific examples
    ...
    Fr_list_A, properties_A = load_fractures(address=output_fol, load_all=True)#time_srs=[.2,.4,.6,0.8,1.2,1.6])#load_all=True)  # load all fractures
    ...
    my_list = []
    mytime = 10.4
    for i in np.arange(0,len(Fr_list_A)-100,ftPntJUMP):
        if Fr_list_A[i].time < mytime:
            my_list.append(Fr_list_A[i])
    for i in np.arange(0, len(Fr_list_B)-100, ftPntJUMP):
