...
if run:
    ...
    def K1c_func(x,y,alpha):
        ...
        r = 1.48
        delta = 0.0005
        return smoothing(K_Ic, 1.2*K_Ic, r, delta, x)
...
if not os.path.isfile('./batch_run.txt'):  # We only visualize for runs of specific examples
    ...
    Fr_list_A, properties_A = load_fractures(address=output_fol,time_srs=[0.4,0.54])#load_all=True)#step_size=20)# load_all=True)  # load all fractures
