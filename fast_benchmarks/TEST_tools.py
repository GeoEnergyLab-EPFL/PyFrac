...
import shutil
import os
from fast_benchmarks.TEST_simulparam_and_tolerances import *
...
def test_(testname, run = True):
    ...
    if not run: delete = False
    else: delete = True

    #load_and_test(outputfolder, my_vertex, toll, delete)

def load_and_test(outputfolder, my_vertex, toll, delete):
    ########################
    ...
    for i in range(len(R_ana_list)):
        ...
        assert Diff < toll['radius_toll'], "At time step n. "+str(i)+" the solution for the mean Radius is far from the analytical more than 5%"
        Diff=np.abs(R_ana_list[i]-Rmax_num_list[i])/R_ana_list[i]
        assert Diff < toll['radius_toll'], "At time step n. "+str(i)+" the solution for the max Radius is far from the analytical more than 5%"
        Diff=np.abs(R_ana_list[i]-Rmin_num_list[i])/R_ana_list[i]
        assert Diff < toll['radius_toll'], "At time step n. "+str(i)+" the solution for the min Radius is far from the analytical more than 5%"
    ...
    for i in range(1,len(w_num_point_values)):
        ...
        assert Diff < toll['w_center_toll'], "At time step n. "+str(i)+" the solution for the Fracture opening is far from the analytical more than 5%"
