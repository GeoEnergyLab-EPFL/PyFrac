...
# creating mesh
#Mesh = CartesianMesh(0.035, 0.03, 85, 85)
#Mesh = CartesianMesh(0.035, 0.035, 99, 99)
#mesh_discretiz=141
mesh_discretiz=183
Mesh = CartesianMesh(0.035, 0.015, mesh_discretiz, mesh_discretiz)
...
def source_location(x, y):
    ...
    """
    # the condition
    if (abs(x + .02) < 1e-5 and abs(y - 0) < 1e-5 ) or (abs(x - .02) < 1e-5 and abs(y + 0) < 1e-5):

        return True

# injection parameters
Q0 =  5/1000/60/1000 #20mL/min  # injection rate, sum of the injections in the single fracture
initialratesecondpoint=Q0/15
...
simulProp.finalTime = 60                     # the time at which the simulation stops
myfolder ="./Data/coalescence_exp_pivdb13_"+str(mesh_discretiz)+"x"+str(mesh_discretiz)
...
simulProp.set_solTimeSeries(np.linspace(21.0, 23.0,300))
...
if run:
    ...
    from pyfrac.fracture_initialization import get_radial_survey_cells
    initRad1 = 0.0019528*0.8
    initRad2 = 0.005505*0.8
    surv_cells_1, surv_cells_dist_1, inner_cells_1 = get_radial_survey_cells(Mesh, initRad1, inj_point=[-0.0195, 0])
    # surv_cells_dist = np.cos(Mesh.CenterCoor[surv_cells, 0]) + 2.5 - abs(Mesh.CenterCoor[surv_cells, 1])

    surv_cells_2, surv_cells_dist_2, inner_cells_2 = get_radial_survey_cells(Mesh, initRad2, inj_point=[0.01885, 0])
...
if export_results:
    ...
    myJsonName="./Data/coalescence_exp_pivdb13_4mma2.json"
    #myfolder="./Data/coalescence_exp_db12_239x121"
    myfolder="./Data/coalescence_exp_pivdb13_141x141"
