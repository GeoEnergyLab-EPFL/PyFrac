...
mesh_discretiz_y=141
#mesh_discretiz_x=205
#mesh_discretiz_y=111
Mesh = CartesianMesh(0.030, 0.015, mesh_discretiz_x, mesh_discretiz_y)
...
def source_location(x, y, sizehx, sizehy):
    ...
    """
    Rinj = 0.00025
    if (abs(x + .02) < sizehx * 0.5 + Rinj and abs(y - 0) < sizehy * 0.5  + Rinj ) or (abs(x - .02) < sizehx*0.5 + Rinj and abs(y + 0) < sizehy*.5 + Rinj):
        return True
...
simulProp.set_solTimeSeries(np.concatenate((np.linspace(0.69, 1.08,100),np.linspace(1.0833, 2.0833,200),np.linspace(2.0833, 3.88,156),np.linspace(18.5, 21.0,35))))
