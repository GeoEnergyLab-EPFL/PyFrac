...
simulProp.finalTime = 25.0               # the time at which the simulation stops
#simulProp.timeStepLimit = 0.01

myfolder ="./Data/coal_VC_exp_db12_"+str(mesh_discretiz_x)+"x"+str(mesh_discretiz_y)
simulProp.set_outputFolder(myfolder)          # the address of the output folder

simulProp.saveToDisk=True
#simulProp.plotVar = ['pf','w']
fixedtimesteps = np.array([[1,8],[0.05,0.05]])
...
simulProp.plotTSJump = 2
simulProp.saveTSJump=3
...
write = True
plot = False
time_slicing=np.linspace(0., 4.,40)
myJsonName = "./Data/coalescence_expdb12_4mma_VC.json"
myfolder ="./Data/coal_VC_exp_db12_"+str(mesh_discretiz_x)+"x"+str(mesh_discretiz_y)
...
if export_results:
    ...
    Fr_list, properties = load_fractures(address="./Data/double_crack_volume_control",
    ...
    Fr_list_COMPLETE, properties = load_fractures(address=myfolder, time_srs=time_slicing)
