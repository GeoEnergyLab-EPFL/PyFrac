...
def findcommon(nodeindex0, nodeindex1, typeindex, connectivityedgeselem, Connectivitynodeselem, edgeORvertexID):
    ...
    diff = np.setdiff1d(cellOfNodei,
                        cellOfNodeip1)  # Return the unique values in cellOfNodei that are not in cellOfNodeip1.
...
def reconstruct_front_continuous(sgndDist_k, anularegion, Ribbon, eltsChannel, mesh):
    # sgndDist_k:
    # vector that contains the distance from the center of the cell to the front. They are negative if the point is inside the fracture otherwise they are positive
    # anularegion:
    # name of the cells where we expect to be the front
    # Ribbon:
    # name of the ribbon elements
    # mesh:
    # obj
                    ...
                    if appendXcandidate == 'yes' and appendYcandidate == 'yes':
                        ...
                        if appendXcandidate == 'yes':
                            ...
                            if (np.any(np.isin(Ribbon, icbaMat[:, Xposition]))):
                                print(
                                    'point forced to be on the vertex because the front is going inside the ribbon cell')
                                typeindex.append(1)
                                edgeORvertexID.append(centernode)
                                xintersection.append(mesh.VertexCoor[centernode, 0])
                                yintersection.append(mesh.VertexCoor[centernode, 1])
    ...
    for nodeindex in range(0, len(xintersection)):
        ...
        if nodeindex == 0:
            commonbackward = findcommon(nodeindex, len(xintersection) - 1, typeindex, mesh.Connectivityedgeselem,
                                        mesh.Connectivitynodeselem, edgeORvertexID)
        else:
            commonbackward = findcommon(nodeindex, (nodeindex - 1), typeindex, mesh.Connectivityedgeselem,
                                        mesh.Connectivitynodeselem, edgeORvertexID)
        # commonforward contains the unique values in cellOfNodei that are in cellOfNodeip1.
        if nodeindex == len(xintersection) - 1:
            commonforward = findcommon(nodeindex, 0, typeindex, mesh.Connectivityedgeselem, mesh.Connectivitynodeselem,
                                       edgeORvertexID)
        else:
            commonforward = findcommon(nodeindex, (nodeindex + 1), typeindex, mesh.Connectivityedgeselem,
                                       mesh.Connectivitynodeselem, edgeORvertexID)
    ...
    for nodeindex in range(0, len(xintersection)):
        ...
        [angle, xint, yint] = findangle(xintersection[nodeindex], yintersection[nodeindex], xintersection[nodeindexp1],
                                        yintersection[nodeindexp1], x, y)
    ...
    return np.asarray(listofTIPcells), np.asarray(listofTIPcellsONLY), np.asarray(distances), np.asarray(
        angles), CellStatusNew, newRibbon, vertexpositionwithinthecell, vertexpositionwithinthecellTIPcellsONLY
