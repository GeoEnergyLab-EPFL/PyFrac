...
with open(filename, "w") as output:
    ...
    for example in example_list:
        ...
        # Running the example
        returnValue = subprocess.call(["python3", example])
        successfullQ = returnValue == 0
        # Summarize working examples to csv
        with open(
            summaryFileName,
            "a",
        ) as summaryFile:
            summaryFile.write(example + ", " + str(successfullQ) + "\n")
