...
from pyfrac.fracture_obj.fracture_initialization import (
    Geometry,
...
def K1c_func(x, y, alpha):
    if alpha > np.pi / 2.0 and alpha <= np.pi:
        alpha = np.pi - alpha
    ...
    K1c_1 = 1.0e6  # fracture toughness along x-axis
...
Solid = MaterialProperties(Mesh, Eprime, anisotropic_K1c=True, K1c_func=K1c_func)
...
simulProp.set_volumeControl(
    True
)  # to set up the solver in volume control mode (inviscid fluid)
...
simulProp.symmetric = (
    True  # solving with faster solver that assumes fracture is symmetric
)
...
gamma = (
    K1c_func(0.0, 0.0, np.pi / 2) / K1c_func(0.0, 0.0, 0.0)
) ** 2  # gamma = (Kc1/Kc3)**2
Fr_geometry = Geometry("elliptical", minor_axis=2.0, gamma=gamma)
...
if not os.path.isfile(
    "./batch_run.txt"
):  # We only visualize for runs of specific examples
    ...
    # plotting footprint
    Fig_FP = plot_fracture_list(Fr_list, variable="mesh", projection="2D")
    Fig_FP = plot_fracture_list(
        Fr_list, variable="footprint", projection="2D", fig=Fig_FP
    )
    Fig_FP = plot_analytical_solution(
        "E_K",
        "footprint",
        Solid,
        Injection,
    ...
    # plotting width
    Fig_w_slice = plot_fracture_list_slice(
        Fr_list,
        variable="width",
    ...
    Fig_w_slice = plot_analytical_solution_slice(
        "E_K",
        variable="width",
        mat_prop=Solid,
        inj_prop=Injection,
    ...
    Fr_list, properties = load_fractures(
        address="./Data/ellipse", sim_name="anisotropic_toughness_benchmark"
    )
    time_srs = get_fracture_variable(Fr_list, variable="time")
    ...
    Fig_len_a = plot_fracture_list(
        Fr_list, variable="d_min", plot_prop=plot_prop, labels=labels
    )
    Fig_len_a = plot_analytical_solution(
        "E_K",
        "d_min",
        Solid,
        Injection,
    ...
    Fig_len_b = plot_fracture_list(
        Fr_list, variable="d_max", plot_prop=plot_prop, labels=labels
    )
    Fig_len_b = plot_analytical_solution(
        "E_K",
        "d_max",
        Solid,
        Injection,
    ...
    Fig_ar = plot_fracture_list(
        Fr_list, variable="ar", plot_prop=plot_prop, labels=labels
    )
