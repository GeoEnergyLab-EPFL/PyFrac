...
Q0 = 0.079 #0.211983  # massive injection rate - 80 BPM
...
simulProp.set_outFileAddress("./Data/MDR_test") # the disk address where the files are saved
simulProp.plotFigure = True
simulProp.plotAnalytical = False
...
Fr_list = load_fractures(simulProp.get_outFileAddress(), multpl_sim=True,
               time_srs=(np.linspace(0.0028,1000,40)))

fig1,fig2=plot_radius(simulProp.get_outFileAddress(), multpl_sim=True,
                        r_type='mean',
                        analytical_sol='K',add_labels=False,anltcl_lnStyle='r')
...
plot_fracture_volume(simulProp.get_outFileAddress())
...
plot_as_matrix(Fr_list[-1].ReynoldsNumber[0],Mesh)


plot_as_matrix(Fr_list[1].ReynoldsNumber[0],Mesh)

PrintDomain(Fr_list[10].ReynoldsNumber[0], Mesh)
...
f1,f2,f3,f4 =plot_profile(simulProp.get_outFileAddress(),multpl_sim=True,
                        plot_at_times=np.linspace(0.0032,1000,6),
                        analytical_sol='K') #,plt_pressure=True

plt.show()


# plot width at early time - compare with MDR
plot_profile(simulProp.get_outFileAddress(),
                        plot_at_times=np.array([ 0.0032,  0.1, 0.32,1,5 ]),#np.linspace(0.0032,0.2,5),
             plt_symbol=".",
             analytical_sol='MDR',anltcl_lnStyle='k')
plt.show()
# how to get the exact corresponding time ?


# plot width at late time - compare with K
plot_profile(simulProp.get_outFileAddress(),
                        plot_at_times=np.array([ 100,200,600,1000 ]),#np.linspace(0.0032,0.2,5),
             plt_symbol=".",
             analytical_sol='K',anltcl_lnStyle='r')
plt.show()
