...
simulProp.saveTSJump, simulProp.plotTSJump = (
    5,
    1,
)  # save and plot after every 5 time steps
...
if not os.path.isfile(
    ...
    Fig_R = plot_fracture_list(
        Fr_list, variable="d_mean", plot_prop=plot_prop
    )  # numerical radius
    ...
    Fr_slice, properties = load_fractures(
        address="./Data/MtoK", time_srs=time_slice
    )  # load specific fractures
    time_slice = get_fracture_variable(Fr_slice, variable="time")

    ext_pnts = np.empty((2, 2), dtype=np.float64)
    Fig_WS_K = plot_fracture_list_slice(
        Fr_slice[3:],
        variable="w",
        projection="2D",
    ...
    Fig_WS_K = plot_analytical_solution_slice(
        "K",
        "w",
        Solid,
        Injection,
    ...
    ext_pnts = np.empty((2, 2), dtype=np.float64)
    Fig_WS_M = plot_fracture_list_slice(
        Fr_slice[:3],
        variable="w",
        projection="2D",
        plot_cell_center=True,
    ...
    # plot slice of width analytical
    Fig_WS_M = plot_analytical_solution_slice(
        "M",
        "w",
        Solid,
        Injection,
