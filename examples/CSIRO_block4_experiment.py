...
if run:
    ...
    Solid = MaterialProperties(
        Mesh, Eprime, K1c, minimum_width=1e-10, Carters_coef=0.0, Carters_t0=None
    )
    ...
    simulProp.fixedTmStp = np.asarray(
        [[0, 15.0, 200.0, 206, 223, 300, 600], [0.4, 1.0, 0.1, 0.03, 1, 10, None]]
    )
    ...
    from pyfrac.solid.elasticity_isotropic import (
    ...
    init_param = InitializationParameters(
        Fr_geometry, regime="static", net_pressure=5e2, elasticity_matrix=C
    )  # time=2.5
    ...
    if restart:
        ...
        Fr_list, properties = load_fractures(
            "./Data/injection_line", sim_name="block4_ts_0.2_r_1.45"
        )
...
Fr_list_complete, properties = load_fractures(
    results_folder, sim_name="block4_ts_0.2_r_1.45"
)
Solid, Fluid, Injection, simulProp = properties
...
Fig_R = plot_fracture_list(Fr_list, variable="footprint", plot_prop=plot_prop)
Fig_R = plot_fracture_list(
    Fr_list,
    fig=Fig_R,
    variable="mesh",
...
Fig_w = plot_fracture_list_at_point(Fr_list, variable="w", plot_prop=plot_prop)
# plot analytical width at center
Fig_w = plot_analytical_solution_at_point(
    "M", "w", Solid, Injection, fluid_prop=Fluid, time_srs=time_srs, fig=Fig_w
)

Fig_w = plot_analytical_solution_at_point(
    "K", "w", Solid, Injection, fluid_prop=Fluid, time_srs=time_srs, fig=Fig_w
)

# plot pressure at center
Fig_pf = plot_fracture_list_at_point(Fr_list, variable="pn", plot_prop=plot_prop)
# plot analytical width at center
Fig_pf = plot_analytical_solution_at_point(
    "M", "pn", Solid, Injection, fluid_prop=Fluid, time_srs=time_srs, fig=Fig_pf
)

Fig_pf = plot_analytical_solution_at_point(
    "K", "pn", Solid, Injection, fluid_prop=Fluid, time_srs=time_srs, fig=Fig_pf
)


# animate_simulation_results(Fr_list, variable=['ir'])
plt_prop = PlotProperties(line_style=".-", line_color="k")
Fig_p = plot_fracture_list(
    Fr_list, variable="injection line pressure", plot_prop=plt_prop
)
