...
Solid = MaterialProperties(Mesh, Eprime, K_Ic, confining_stress_func=sigmaO_func)
...
simulProp.finalTime = 145.0  # the time at which the simulation stops
simulProp.bckColor = (
    "sigma0"  # setting the parameter according to which the mesh is color coded
)
...
Fr_geometry = Geometry(shape="radial", radius=1.0)
...
if not os.path.isfile(
    ...
    Fig_r = plot_analytical_solution(
        "PKN",
        variable="d_max",
        mat_prop=Solid,
        inj_prop=Injection,
    ...
    plot_prop = PlotProperties(line_style=".", graph_scaling="loglog")
    Fig_w = plot_fracture_list_at_point(
        Fr_list, variable="w", plot_prop=plot_prop, labels=label  # plotting footprint
    )
    ...
    Fig_Fr = plot_analytical_solution(
        "PKN",
        variable="footprint",
        mat_prop=Solid,
        inj_prop=Injection,
    ...
    # Fig_Fr = None
    Fig_Fr = plot_fracture_list(
        Fr_list, variable="footprint", projection="3D", fig=Fig_Fr  # plotting footprint
    )

    plot_prop = PlotProperties(alpha=0.2, text_size=5)  # plotting width
    Fig_Fr = plot_fracture_list(
        Fr_list, variable="w", projection="3D", fig=Fig_Fr, plot_prop=plot_prop
    )
